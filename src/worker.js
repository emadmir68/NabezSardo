...
async function runCutoverSelfTest(request) {
  if (request.headers.get("x-nabzesardo-cutover") !== "cutover-7d4c43e2a9f641fd9b2d") {
  ...
  try {
    const auth = await runtimeAuth();
    if (!auth) throw new Error("admin-auth-not-ready");
    ...
    const loginBody = new URLSearchParams({
      ...
      password: String(auth.verifier)
    }).toString();
    const loginRes = await handleApp(new Request(new URL("/admin/login", base), {
      method: "POST",
    ...
    checks.login = loginRes.status === 302 && cookie.startsWith("nabez_admin=");
    if (!checks.login) throw new Error("admin-login-selftest-failed");

    const dashRes = await handleApp(new Request(new URL("/admin", base), {
      headers: { cookie }
    }));
    const dashText = await dashRes.text();
    checks.dashboard = dashRes.status === 200 && /داشبورد|خبر|مدیریت/.test(dashText);
    ...
    const createRes = await handleApp(new Request(new URL("/admin/articles/new", base), {
      method: "POST",
    ...
    const deleteRes = await handleApp(new Request(new URL("/admin/articles/" + encodeURIComponent(tempArticleId) + "/delete", base), {
      method: "POST",
      headers: { cookie, "content-type": "application/x-www-form-urlencoded" },
      body: "",
      redirect: "manual"
    }));
    db = await loadDbObject();
    checks.draftDelete = deleteRes.status === 302 && !(db.articles || []).some(a => String(a.id) === tempArticleId);
    if (!checks.draftDelete) throw new Error("draft-delete-selftest-failed");
    tempArticleId = "";

    const png = base64Bytes("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl2nYQAAAAASUVORK5CYII=");
    const form = new FormData();
    form.append("image", new Blob([png], { type: "image/png" }), "cutover-test.png");
    const uploadRes = await handleApp(new Request(new URL("/admin/upload/image", base), {
      method: "POST",
      headers: { cookie },
      body: form
    }));
    const uploadJson = await uploadRes.json().catch(() => null);
    tempMediaName = uploadJson && uploadJson.url ? path.basename(String(uploadJson.url)) : "";
    const storedMedia = tempMediaName ? await getMedia(tempMediaName) : null;
    checks.mediaUpload = uploadRes.status === 200 && Boolean(storedMedia && storedMedia.data && storedMedia.data.length);
    if (!checks.mediaUpload) throw new Error("media-upload-selftest-failed");
    await deleteMedia(tempMediaName);
    checks.mediaDelete = !(await getMedia(tempMediaName));
    tempMediaName = "";

    const runtime = await decryptRuntimeSecrets();
    checks.integrations = {
      telegram: Boolean(runtime.TELEGRAM_BOT_TOKEN && runtime.TELEGRAM_CHAT_ID),
      rubika: Boolean(runtime.RUBIKA_BOT_TOKEN && runtime.RUBIKA_CHAT_ID),
      whatsapp: Boolean(runtime.WHATSAPP_ACCESS_TOKEN && runtime.WHATSAPP_PHONE_NUMBER_ID && runtime.WHATSAPP_TO)
    };

    return Response.json({ ok: true, primary: isPrimary(), checks });
  } catch (err) {
    try {
      if (tempMediaName) await deleteMedia(tempMediaName);
      if (tempArticleId) {
        const db = await loadDbObject();
        db.articles = (db.articles || []).filter(a => String(a.id) !== tempArticleId);
        await putState("db", JSON.stringify(db, null, 2));
      }
    } catch {}
    return Response.json({ ok: false, error: String(err?.message || err), checks }, { status: 500 });
  }
}
...
export default {
  async fetch(request) {
    ...
    if (p === "/__cutover/selftest" && request.method === "GET") return runCutoverSelfTest(request);
