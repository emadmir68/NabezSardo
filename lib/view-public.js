...
function siteSchema(){
  return [
    ...
    {'@context':'https://schema.org','@type':'NewsMediaOrganization',name:'نبض ساردو',alternateName:'Nabez Sardo',url:PUBLIC_BASE+'/',description:'رسانه محلی ساردوئیه، جیرفت و جنوب کرمان'}
  ];
}
function articleSchema(a,c){
  ...
  const news={'@context':'https://schema.org','@type':'NewsArticle',mainEntityOfPage:{'@type':'WebPage','@id':url},headline:a.title,description:a.lead||a.title,datePublished:a.publishedAt||a.createdAt,dateModified:a.updatedAt||a.publishedAt||a.createdAt,inLanguage:'fa-IR',isAccessibleForFree:true,articleSection:c?.name||'خبر',author:{'@type':authorType,name:authorName},publisher:{'@type':'NewsMediaOrganization',name:'نبض ساردو',url:PUBLIC_BASE+'/'},url};
...
function home(db){
 ...
 return shell('نبض ساردو | اخبار ساردوئیه و جنوب کرمان',`${header(db)}
...
function archive(db,q=''){const all=published(db).filter(a=>!q||a.title.includes(q)||a.lead.includes(q));return shell('آرشیو اخبار | نبض ساردو',`${header(db)}<main class="wrap page"><div class="pagehead"><span class="eyebrow">ARCHIVE</span><h1>آرشیو اخبار</h1></div><form class="search" action="/all-news"><input name="q" value="${e(q)}" placeholder="جستجو در خبرها"><button class="btn primary">جستجو</button></form><div class="grid news">${all.map(a=>card(a,db)).join('')||'<div class="empty">خبری پیدا نشد.</div>'}</div></main>${footer()}`,'آرشیو کامل اخبار نبض ساردو',{canonical:'/all-news'});}
function article(db,a){const c=db.categories.find(x=>x.id===a.categoryId);const body=a.bodyHtml||oldBody(a);const gallery=Array.isArray(a.gallery)?a.gallery:[];return shell(a.title+' | نبض ساردو',`${header(db)}<main class="wrap article"><article><span class="eyebrow">${e(c?.name||'خبر')}</span><h1>${e(a.title)}</h1><p class="lead">${e(a.lead||'')}</p><div class="meta"><span>${e(a.author||'تحریریه نبض ساردو')}</span><span>${e(a.location||'ساردوئیه')}</span><span>${new Intl.DateTimeFormat('fa-IR',{dateStyle:'medium',timeStyle:'short',timeZone:'Asia/Tehran'}).format(new Date(a.publishedAt||a.createdAt))}</span><span class="article-views">◉ ${Number(a.views||0).toLocaleString('fa-IR')} بازدید</span></div>${a.image?`<div class="cover-frame ${imageOrientation(a)}" data-adaptive-media style="--image-ratio:${imageRatio(a)}"><img class="cover" src="${e(a.image)}" alt="${e(a.title)}"></div>`:''}<div class="body rich-content">${body}</div>${gallery.length?`<section class="article-gallery"><h3>گالری تصاویر</h3><div class="article-gallery-grid">${gallery.map((url,i)=>`<img src="${e(url)}" alt="${e(a.title)} - تصویر ${i+1}">`).join('')}</div></section>`:''}</article></main>${footer()}`,a.lead||a.title,{canonical:'/news/'+encodeURIComponent(a.slug),image:a.image||'',ogType:'article',schema:articleSchema(a,c)});}
...
function category(db,c){const list=published(db).filter(a=>a.categoryId===c.id);return shell(c.name+' | نبض ساردو',`${header(db)}<main class="wrap page"><span class="eyebrow">CATEGORY</span><h1>${e(c.name)}</h1><div class="grid news">${list.map(a=>card(a,db)).join('')||'<div class="empty">هنوز خبری در این دسته منتشر نشده است.</div>'}</div></main>${footer()}`,'اخبار '+c.name+' در نبض ساردو',{canonical:'/category/'+encodeURIComponent(c.id)});}
