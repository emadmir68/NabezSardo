...
function article(db,a){
  ...
  const quick5=compactText(a.lead||plainBody||a.title,170);
  const quick30=compactText([a.lead,plainBody].filter(Boolean).join(' '),760);
  const quick5En=compactText(a.leadEn||a.lead||plainBody||a.title,190);
  const quick30En=compactText([a.leadEn,a.bodyEn].filter(Boolean).join(' ')||[a.lead,plainBody].filter(Boolean).join(' '),780);
  const quickWhere=a.location||'ساردوئیه';
  const quickTopic=c?.name||'خبر';
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(shareUrl)}" data-article-canonical="${e(articleUrl)}" data-article-image="${e(a.image||'')}" data-article-lead="${e(shareLead)}">
      ...
      <section class="story-depth" data-story-depth data-depth-mode="2">
        <div class="story-depth-head">
          <div><span class="eyebrow">NABEZ QUICK READ</span><h2><span class="lang-fa">چقدر وقت داری؟</span><span class="lang-en">How much time do you have?</span></h2><p><span class="lang-fa">با کشیدن اسلایدر، عمق همین خبر را خودت انتخاب کن.</span><span class="lang-en">Drag to choose how deeply you want to read this story.</span></p></div>
          <div class="story-depth-now"><strong data-depth-big><span class="lang-fa">کامل</span><span class="lang-en">FULL</span></strong><small data-depth-caption><span class="lang-fa">خبر کامل</span><span class="lang-en">Full story</span></small></div>
        </div>
        <div class="story-depth-control">
          <div class="story-depth-options" role="tablist" aria-label="مدت مطالعه خبر">
            <button type="button" role="tab" aria-selected="false" data-depth-option="0"><b>۵</b><span><span class="lang-fa">ثانیه</span><span class="lang-en">SEC</span></span></button>
            <button type="button" role="tab" aria-selected="false" data-depth-option="1"><b>۳۰</b><span><span class="lang-fa">ثانیه</span><span class="lang-en">SEC</span></span></button>
            <button type="button" role="tab" aria-selected="true" class="is-active" data-depth-option="2"><b>∞</b><span><span class="lang-fa">کامل</span><span class="lang-en">FULL</span></span></button>
          </div>
          <div class="story-depth-range-wrap"><input class="story-depth-range" data-depth-range type="range" min="0" max="2" step="1" value="2" aria-label="انتخاب مدت مطالعه"><div class="story-depth-track" aria-hidden="true"><i></i><span></span><span></span><span></span></div></div>
        </div>
        <div class="story-depth-preview" data-depth-preview="0" hidden>
          <div class="story-depth-fast-grid">
            <div class="story-depth-fast-main"><small><span class="lang-fa">اصل خبر</span><span class="lang-en">The story</span></small><p><span class="lang-fa">${e(quick5)}</span><span class="lang-en">${e(quick5En)}</span></p></div>
            <div><small><span class="lang-fa">کجا؟</span><span class="lang-en">Where?</span></small><b>${e(quickWhere)}</b></div>
            <div><small><span class="lang-fa">موضوع</span><span class="lang-en">Topic</span></small><b>${e(quickTopic)}</b></div>
          </div>
        </div>
        <div class="story-depth-preview story-depth-preview-30" data-depth-preview="1" hidden><span class="story-depth-time-badge">۳۰ SEC</span><p><span class="lang-fa">${e(quick30)}</span><span class="lang-en">${e(quick30En)}</span></p></div>
      </section>
      <div class="article-reading-zone" data-depth-full>
