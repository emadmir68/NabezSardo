...
function articleSchema(a,c){
  ...
  const hay=[a.title,a.lead,a.body,a.location,c?.name].filter(Boolean).join(' ');
  ...
  const news={'@context':'https://schema.org','@type':'NewsArticle',mainEntityOfPage:{'@type':'WebPage','@id':url},headline:a.title,description:compactText(a.lead||a.body||a.title,220),datePublished:a.publishedAt||a.createdAt,dateModified:a.updatedAt||a.publishedAt||a.createdAt,inLanguage:'fa-IR',isAccessibleForFree:true,articleSection:c?.name||'خبر',keywords:[c?.name,a.location,'نبض ساردو','جنوب کرمان',...matchedLocal].filter(Boolean).filter((v,i,arr)=>arr.indexOf(v)===i).join(', '),author:{'@type':authorType,name:authorName},publisher:{'@type':'NewsMediaOrganization',name:'نبض ساردو',url:PUBLIC_BASE+'/'},url};
