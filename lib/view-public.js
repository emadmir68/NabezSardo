...
function home(db){
 ...
 <main><section class="hero hero-photo-final"><div class="hero-photo-layer" aria-hidden="true"><img src="/hero-mosque.jpg?v=verified-mosque-2" alt="" fetchpriority="high" decoding="sync" onerror="this.style.display='none'"></div><div class="wrap hero-grid"><div class="hero-copy reveal"><span class="chip">3D EDITORIAL · SARDOUIYEH</span><h1>نبضِ ساردوئیه،<br>با یک هویت رسانه‌ای تازه</h1><p>خبر محلی، گزارش میدانی، روایت تصویری و صدای مردم؛ در یک تجربه خبری مدرن و متفاوت.</p><div class="actions"><a class="btn gold" href="#latest">تازه‌ترین خبرها</a><a class="textlink" href="/about">درباره تحریریه</a></div></div><div class="hero-art reveal"><div class="pulse-stage" aria-hidden="true"><span class="pulse-halo pulse-halo-a"></span><span class="pulse-halo pulse-halo-b"></span><svg class="lux-pulse" viewBox="0 0 520 260" focusable="false"><defs><linearGradient id="pulseGold" x1="24" y1="80" x2="494" y2="178" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#8d6328"/><stop offset=".18" stop-color="#f5d995"/><stop offset=".5" stop-color="#fff1c2"/><stop offset=".78" stop-color="#d7a956"/><stop offset="1" stop-color="#75501e"/></linearGradient><filter id="pulseSoft" x="-30%" y="-80%" width="160%" height="260%"><feGaussianBlur stdDeviation="9"/></filter><filter id="pulseShine" x="-30%" y="-80%" width="160%" height="260%"><feGaussianBlur stdDeviation="2.3"/></filter></defs><path class="pulse-shadow" d="M24 146H138L166 114L198 185L239 63L281 191L320 119L352 146H496" fill="none" stroke="#d7a956" stroke-width="15" stroke-linecap="round" stroke-linejoin="round" filter="url(#pulseSoft)"/><path class="pulse-main" d="M24 146H138L166 114L198 185L239 63L281 191L320 119L352 146H496" fill="none" stroke="url(#pulseGold)" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/><path class="pulse-highlight" d="M24 144H138L166 112L198 183L239 61L281 189L320 117L352 144H496" fill="none" stroke="#fff7db" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" filter="url(#pulseShine)"/><circle class="pulse-node" cx="239" cy="63" r="7" fill="#fff0bd"/></svg><div class="pulse-caption"><span>LIVE / SARDOUIYEH</span><b>نبض خبرهای منطقه</b></div></div></div></div></div></section>
...
function article(db,a){
  ...
  return shell(a.title+' | نبض ساردو',`${header(db)}<main class="wrap article"><nav class="breadcrumbs" aria-label="مسیر صفحه"><a href="/">خانه</a><span>›</span><a href="${catUrl}"><span class="lang-fa">${e(c?.name||'خبر')}</span><span class="lang-en">${e(catEn(c?.name||'News'))}</span></a></nav><article><span class="eyebrow"><span class="lang-fa">${e(c?.name||'خبر')}</span><span class="lang-en">${e(catEn(c?.name||'News'))}</span></span><h1><span class="lang-fa">${e(a.title)}</span><span class="lang-en">${e(a.titleEn||a.title)}</span></h1><p class="lead"><span class="lang-fa">${e(a.lead||'')}</span><span class="lang-en">${e(a.leadEn||a.lead||'')}</span></p><div class="meta"><span>${e(a.author||'تحریریه نبض ساردو')}</span><span>${e(a.location||'ساردوئیه')}</span><time class="news-date" data-news-date="${e(a.publishedAt||a.createdAt||'')}"></time><span class="article-views">◉ ${Number(a.views||0).toLocaleString('fa-IR')} <span class="lang-fa">بازدید</span><span class="lang-en">views</span></span></div>${a.image?`<div class="cover-frame ${imageOrientation(a)}" data-adaptive-media style="--image-ratio:${imageRatio(a)}"><img class="cover" src="${e(a.image)}" alt="${e(a.title)}"></div>`:''}<div class="body rich-content"><div class="lang-fa">${body}</div><div class="lang-en">${bodyEn}</div></div>${gallery.length?`<section class="article-gallery"><h3>گالری تصاویر</h3><div class="article-gallery-grid">${gallery.map((url,i)=>`<img src="${e(url)}" alt="${e(a.title)} - تصویر ${i+1}">`).join('')}</div></section>`:''}</article>${related.length?`<section class="related-news"><div class="sectionhead"><div><span class="eyebrow">RELATED</span><h2>خبرهای مرتبط</h2></div><a class="more" href="${catUrl}">همه خبرهای این بخش</a></div><div class="grid news">${related.map(x=>card(x,db)).join('')}</div></section>`:''}</main>${footer()}`,desc,{canonical:'/news/'+encodeURIComponent(a.slug),image:a.image||'',ogType:'article',schema:articleSchema(a,c)});
}
