...
/* Experimental homepage module; remove this flag + block for a clean rollback. */
const NEWS_RADAR_ENABLED=true;
...
function home(db){
 ...
 const radarPositions=[
   {x:50,y:17,size:15},{x:76,y:31,size:12},{x:80,y:63,size:14},
   {x:56,y:79,size:11},{x:26,y:67,size:13},{x:22,y:34,size:10}
 ];
 const radarCandidates=regularNews.slice(0,24).map((a,order)=>{
   const publishedAt=a.publishedAt||a.createdAt||'';
   const ts=Date.parse(publishedAt)||0;
   const ageHours=ts?Math.max(0,(Date.now()-ts)/3600000):240;
   const freshness=Math.max(0,96-Math.min(96,ageHours))*4.2;
   const attention=Math.log10(Number(a.views||0)+1)*42;
   const featuredBoost=a.featured?72:0;
   const orderBoost=Math.max(0,24-order)*2.5;
   return {a,score:freshness+attention+featuredBoost+orderBoost};
 }).sort((x,y)=>y.score-x.score);
 const radarNews=radarCandidates.slice(0,6).map(x=>x.a);
 const radarSignature=radarNews.map(a=>[a.id||a.slug,a.publishedAt||a.createdAt||'',Number(a.views||0)].join(':')).join('|');
 const radarPointsMarkup=radarNews.map((a,i)=>{
   const p=radarPositions[i]||radarPositions[0];
   const c=categoryOf(a);
   const url=storyUrl(a);
   return `<a class="news-radar-point ${i===0?'is-prime':''}" href="${url}" style="--rx:${p.x}%;--ry:${p.y}%;--rsize:${p.size}px;--rdelay:${(i*.37).toFixed(2)}s" aria-label="${e(a.title)}">
     <span class="news-radar-ping"></span>
     <i></i>
     <b>${String(i+1).padStart(2,'0')}</b>
     <span class="news-radar-tip"><small>${e(c?.name||'خبر')}</small><strong><span class="lang-fa">${e(compactText(a.title,82))}</span><span class="lang-en">${e(compactText(a.titleEn||a.title,82))}</span></strong></span>
   </a>`;
 }).join('');
 const radarListMarkup=radarNews.map((a,i)=>{
   const c=categoryOf(a);
   const url=storyUrl(a);
   return `<a class="news-radar-item ${i===0?'is-hot':''}" href="${url}">
     <span class="news-radar-item-rank">${String(i+1).padStart(2,'0')}</span>
     <span class="news-radar-item-copy">
       <small><i></i><span class="lang-fa">${e(c?.name||'خبر')}</span><span class="lang-en">${e(catEn(c?.name||'News'))}</span></small>
       <strong><span class="lang-fa">${e(a.title)}</span><span class="lang-en">${e(a.titleEn||a.title)}</span></strong>
       <em><span>◉ ${Number(a.views||0).toLocaleString('fa-IR')}</span><time class="news-date" data-news-date="${e(a.publishedAt||a.createdAt||'')}"></time></em>
     </span>
     <span class="news-radar-item-arrow" aria-hidden="true">←</span>
   </a>`;
 }).join('');
 ...
 ${NEWS_RADAR_ENABLED&&radarNews.length?`<section class="wrap news-radar-section" data-news-radar data-radar-signature="${e(radarSignature)}" aria-label="رادار خبر">
   <div class="news-radar-shell">
     <div class="news-radar-head">
       <div class="news-radar-title">
         <span class="news-radar-kicker"><i></i> NEWS RADAR / HOT NOW</span>
         <h2><span class="lang-fa">رادار خبر</span><span class="lang-en">News Radar</span></h2>
         <p><span class="lang-fa">خبرهای داغ و تازه را روی رادار زنده نبض ساردو دنبال کنید.</span><span class="lang-en">Track fresh and trending stories on the Nabez Sardo live radar.</span></p>
       </div>
       <div class="news-radar-live">
         <span><i></i> LIVE SCAN</span>
         <small><b>${radarNews.length.toLocaleString('fa-IR')}</b> <span class="lang-fa">خبر روی رادار</span><span class="lang-en">stories on radar</span></small>
         <time data-radar-clock></time>
       </div>
     </div>
     <div class="news-radar-grid">
       <div class="news-radar-stage" aria-label="نمایش تعاملی خبرهای داغ">
         <div class="news-radar-cross cross-x" aria-hidden="true"></div>
         <div class="news-radar-cross cross-y" aria-hidden="true"></div>
         <div class="news-radar-ring ring-1" aria-hidden="true"></div>
         <div class="news-radar-ring ring-2" aria-hidden="true"></div>
         <div class="news-radar-ring ring-3" aria-hidden="true"></div>
         <div class="news-radar-ring ring-4" aria-hidden="true"></div>
         <div class="news-radar-sweep" aria-hidden="true"></div>
         <div class="news-radar-core" aria-hidden="true"><span></span><b>N</b><small>LIVE</small></div>
         <div class="news-radar-points">${radarPointsMarkup}</div>
         <div class="news-radar-coordinate top">N 29°</div>
         <div class="news-radar-coordinate bottom">S 27°</div>
         <div class="news-radar-coordinate left">58° E</div>
         <div class="news-radar-coordinate right">57° E</div>
       </div>
       <aside class="news-radar-feed">
         <div class="news-radar-feed-head">
           <span><span class="lang-fa">سیگنال‌های داغ</span><span class="lang-en">Hot signals</span></span>
           <i></i>
           <small>AUTO REFRESH</small>
         </div>
         <div class="news-radar-list">${radarListMarkup}</div>
       </aside>
     </div>
   </div>
 </section>`:''}
