...
const {e,shell,header,footer,card,oldBody,imageRatio,imageOrientation,absoluteUrl,PUBLIC_BASE,bi,newsDate,catEn,categoryTone}=require('./view-common');
...
function siteSchema(){
  return [
    ...
    {'@context':'https://schema.org','@type':'NewsMediaOrganization',name:'نبض ساردو',alternateName:'Nabez Sardo',url:PUBLIC_BASE+'/',logo:absoluteUrl('/assets/logo.svg'),description:'رسانه محلی ساردو، ساردوئیه، جیرفت، عنبرآباد، کهنوج و جنوب کرمان',areaServed:[{'@type':'Place',name:'ساردوئیه'},{'@type':'Place',name:'جیرفت'},{'@type':'Place',name:'عنبرآباد'},{'@type':'Place',name:'کهنوج'},{'@type':'Place',name:'جنوب کرمان'}]}
  ];
}
function articleSchema(a,c){
  ...
  const news={'@context':'https://schema.org','@type':'NewsArticle',mainEntityOfPage:{'@type':'WebPage','@id':url},headline:a.title,description:compactText(a.lead||a.body||a.title,220),datePublished:a.publishedAt||a.createdAt,dateModified:a.updatedAt||a.publishedAt||a.createdAt,inLanguage:'fa-IR',isAccessibleForFree:true,articleSection:c?.name||'خبر',keywords:[c?.name,a.location,'نبض ساردو','جنوب کرمان'].filter(Boolean).join(', '),author:{'@type':authorType,name:authorName,...(authorType==='Organization'?{url:absoluteUrl('/about')}: {})},publisher:{'@type':'NewsMediaOrganization',name:'نبض ساردو',url:PUBLIC_BASE+'/',logo:{'@type':'ImageObject',url:absoluteUrl('/assets/logo.svg')}},url};
...
const LOCAL_HUBS={
  sardouiyeh:{name:'ساردوئیه',en:'Sardouiyeh',terms:['ساردوئیه','ساردویه','ساردو'],description:'آخرین اخبار ساردوئیه؛ رویدادهای محلی، اجتماعی، کشاورزی، گردشگری و مطالبات مردم.',context:'این صفحه فقط خبرهایی را گردآوری می‌کند که محل خبر، تیتر یا لید آن‌ها مستقیماً به ساردوئیه و ساردو مرتبط باشد.'},
  jiroft:{name:'جیرفت',en:'Jiroft',terms:['جیرفت'],description:'آخرین اخبار جیرفت؛ تازه‌ترین رویدادهای شهرستان جیرفت و جنوب کرمان.',context:'پوشش خبرهای شهری، روستایی، اجتماعی، اقتصادی و خدماتی شهرستان جیرفت با تمرکز بر رویدادهای محلی.'},
  anbarabad:{name:'عنبرآباد',en:'Anbarabad',terms:['عنبرآباد','عنبر آباد','عنبر اباد'],description:'آخرین اخبار عنبرآباد؛ رویدادهای شهری و روستایی، کشاورزی، اقتصاد، جامعه و مطالبات مردم.',context:'این صفحه آرشیو اختصاصی خبرهای مرتبط با عنبرآباد است و با انتشار خبرهای جدید منطقه به‌صورت خودکار کامل‌تر می‌شود.'},
  kahnuj:{name:'کهنوج',en:'Kahnuj',terms:['کهنوج'],description:'آخرین اخبار کهنوج؛ تازه‌ترین رویدادهای شهرستان کهنوج و جنوب کرمان.',context:'خبرهای مرتبط با کهنوج، خدمات عمومی، جامعه، اقتصاد، کشاورزی و رویدادهای شهرستان در این صفحه تجمیع می‌شوند.'},
  'south-kerman':{name:'جنوب کرمان',en:'South Kerman',terms:['جنوب کرمان','کرمان جنوبی','ساردوئیه','ساردویه','جیرفت','عنبرآباد','عنبر آباد','کهنوج','رودبار جنوب','قلعه گنج','قلعه‌گنج','منوجان','فاریاب'],description:'آخرین اخبار جنوب کرمان؛ پوشش خبرهای شهرستان‌ها، اقتصاد، کشاورزی، جامعه و رویدادهای منطقه.',context:'این صفحه فقط خبرهایی را نمایش می‌دهد که در محل خبر، تیتر یا لید به شهرستان‌ها و مناطق جنوب کرمان اشاره مستقیم داشته باشند؛ خبرهای ملی و نامرتبط صرفاً به‌دلیل اشاره در متن وارد این آرشیو نمی‌شوند.'}
};
function localMatchesArticle(a,hub){
  const normalize=v=>String(v||'').replace(/\u200c/g,' ').replace(/\s+/g,' ').trim();
  const terms=hub.terms.map(normalize);
  const location=normalize(a.location);
  const headline=normalize([a.title,a.lead].filter(Boolean).join(' '));
  const has=text=>Boolean(text)&&terms.some(t=>text.includes(t));
  return has(location)||has(headline);
}
function localHub(db,key){
  const hub=LOCAL_HUBS[key];if(!hub)return null;
  const list=published(db).filter(a=>localMatchesArticle(a,hub));
  ...
  const localLinks=Object.entries(LOCAL_HUBS).filter(([k])=>k!==key).map(([k,x])=>'<a class="more" href="/local/'+encodeURIComponent(k)+'">اخبار '+e(x.name)+'</a>').join(' · ');
  const update=list[0]?'<p class="meta">آخرین بروزرسانی: '+newsDate(list[0].publishedAt||list[0].createdAt||'')+'</p>':'';
  const body=header(db)+'<main class="wrap page category-page local-hub-page"><span class="eyebrow">LOCAL NEWS / '+e(hub.en)+'</span><h1>آخرین اخبار '+e(hub.name)+'</h1><p class="lead">'+e(hub.description)+'</p><section class="panel local-hub-context"><h2>پوشش خبری '+e(hub.name)+'</h2><p>'+e(hub.context)+'</p><p>تعداد خبرهای مرتبط در این آرشیو: <strong>'+Number(list.length).toLocaleString('fa-IR')+'</strong></p>'+update+'<nav aria-label="سایر صفحات محلی">'+localLinks+'</nav></section><div class="grid news">'+(list.map(a=>card(a,db)).join('')||'<div class="empty">هنوز خبری برای این منطقه منتشر نشده است.</div>')+'</div></main>'+footer();
  return shell('اخبار '+hub.name+' امروز | نبض ساردو',body,hub.description,{canonical,schema,robots:list.length?'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1':'noindex,follow'});
}
function home(db){
 ...
 const leadMarkup=leadStory?(()=>{
   ...
   return `<article class="front-lead${/(?:^|\/)auto-cover-[^/]+\.(?:svg|webp|png|jpe?g)(?:\?|$)/i.test(image)?' auto-cover-story':''}" data-cat-tone="${e(categoryTone(c))}">
     ${media}
     <div class="front-lead-copy">
       ...
       <h2><a href="${url}">${bi(leadStory.title,leadStory.titleEn||'')}</a></h2>
       <p>${bi(compactText(leadStory.lead||leadStory.body||'',185),compactText(leadStory.leadEn||'',185))}</p>
       <div class="front-lead-bottom">${newsDate(leadStory.publishedAt||leadStory.createdAt||'')}<a class="more" href="${url}"><span class="lang-fa">ادامه خبر ←</span><span class="lang-en">Read story →</span></a></div>
     </div>
   </article>`;
 ...
 const highlightsMarkup=highlightNews.map((a,i)=>{
   ...
   return `<article class="front-brief" data-cat-tone="${e(categoryTone(c))}">
     ...
     <div class="front-brief-copy">
       ...
       <h3><a href="${url}">${bi(a.title,a.titleEn||'')}</a></h3>
       ${newsDate(a.publishedAt||a.createdAt||'')}
     </div>
   </article>`;
 }).join('');
 const popularMarkup=popular.map((a,i)=>{
   ...
   return `<article class="popular-row" data-cat-tone="${e(categoryTone(c))}">
     ...
     <div><span class="eyebrow"><span class="lang-fa">${e(c?.name||'خبر')}</span><span class="lang-en">${e(catEn(c?.name||'News'))}</span></span><h3><a href="${url}">${bi(a.title,a.titleEn||'')}</a></h3><div class="popular-meta"><span>◉ ${Number(a.views||0).toLocaleString('fa-IR')}</span>${newsDate(a.publishedAt||a.createdAt||'')}</div></div>
   </article>`;
 }).join('');
 const todayPulseMarkup=todayPulse.map((a,i)=>{
   ...
   return `<article class="today-pulse-item" data-cat-tone="${e(categoryTone(c))}">
     ...
     <div class="today-pulse-content">
       ...
       <h3><a href="${url}">${bi(a.title,a.titleEn||'')}</a></h3>
       <div class="today-pulse-bottom">${newsDate(a.publishedAt||a.createdAt||'')}<a href="${url}" class="today-pulse-link"><span class="lang-fa">مشاهده خبر ←</span><span class="lang-en">Read →</span></a></div>
     </div>
   </article>`;
 }).join('');
 const shortNewsMarkup=shortNews.map((a,i)=>{
   ...
   const cat=db.categories.find(x=>x.id===a.categoryId);
   return `<article class="short-timeline-item" data-cat-tone="${e(categoryTone(cat))}">
     <div class="short-timeline-rail" aria-hidden="true">
       <span class="short-timeline-no">${String(i+1).padStart(2,'0')}</span>
     </div>
     <div class="short-timeline-content">
       <div class="short-timeline-meta">
         <span class="short-timeline-cat"><span class="lang-fa">${e(cat?.name||'خبر')}</span><span class="lang-en">${e(catEn(cat?.name||'News'))}</span></span>
         <span class="short-timeline-views">◉ ${Number(a.views||0).toLocaleString('fa-IR')}</span>
       </div>
       <h3><a href="${url}">${bi(a.title,a.titleEn||'')}</a></h3>
       <div class="short-timeline-bottom">
         ${newsDate(date)}
         <a class="short-timeline-link" href="${url}"><span class="lang-fa">مشاهده خبر ←</span><span class="lang-en">Read story →</span></a>
       </div>
     </div>
   </article>`;
 }).join('');
...
function article(db,a){
  ...
  const location=e(a.location||'ساردوئیه');
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(shareUrl)}" data-article-canonical="${e(articleUrl)}" data-article-image="${e(a.image||'')}" data-article-lead="${e(shareLead)}">
      <header class="article-head">
        ...
        <h1>${bi(a.title,a.titleEn||'')}</h1>
        <div class="article-trustline"><span><b><span class="lang-fa">نویسنده:</span><span class="lang-en">By:</span></b> ${author}</span><span aria-hidden="true">•</span><span><b><span class="lang-fa">انتشار:</span><span class="lang-en">Published:</span></b> ${newsDate(a.publishedAt||a.createdAt||'')}</span></div>
        <p class="lead article-deck">${bi(a.lead||'',a.leadEn||'')}</p>
      ...
      ${a.image?`<figure class="article-cover-wrap${/(?:^|\/)auto-cover-[^/]+\.(?:svg|webp|png|jpe?g)(?:\?|$)/i.test(a.image)?' auto-cover-story':''}"><div class="cover-frame ${imageOrientation(a)}" data-adaptive-media style="--image-ratio:${imageRatio(a)}"><img class="cover" src="${e(a.image)}" alt="${e(a.title)}" loading="eager" decoding="async" fetchpriority="high"></div><figcaption><span class="lang-fa">تصویر خبر · نبض ساردو</span><span class="lang-en">News image · Nabez Sardo</span></figcaption></figure>`:''}
      ...
      <div class="article-reading-zone">
        ...
        <div class="body rich-content article-body">${bodyMarkup}</div>
      </div>
      ${gallery.length?`<section class="article-gallery premium-gallery"><div class="article-gallery-head"><span class="eyebrow">GALLERY</span><h3><span class="lang-fa">گالری تصاویر</span><span class="lang-en">Photo Gallery</span></h3></div><div class="article-gallery-grid">${gallery.map((url,i)=>`<img src="${e(url)}" alt="${e(a.title)} - تصویر ${i+1}" loading="lazy" decoding="async" fetchpriority="low">`).join('')}</div></section>`:''}
      <div class="article-endnote"><svg viewBox="0 0 230 42" aria-hidden="true"><path d="M3 23h48l10-8 11 20 14-32 15 33 14-22 13 9h99" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/></svg><div><b><span class="lang-fa">پایان خبر</span><span class="lang-en">End of story</span></b><small>NABEZ SARDO · LOCAL NEWSROOM</small></div></div>
        <div class="article-meta-grid">
          ...
          <div class="article-meta-item"><span class="article-meta-icon" aria-hidden="true">⌖</span><small><span class="lang-fa">محل خبر</span><span class="lang-en">Location</span></small><b>${location}</b></div>
          <div class="article-meta-item"><span class="article-meta-icon" aria-hidden="true">◷</span><small><span class="lang-fa">انتشار</span><span class="lang-en">Published</span></small>${newsDate(a.publishedAt||a.createdAt||'')}</div>
  ...
  </main>${footer()}`,desc,{canonical:articleUrl,image:a.image||'',ogType:'article',schema:articleSchema(a,c)});
}
...
function category(db,c){
  ...
  if(c.id==='short-news'){
    const rows=list.map((a,i)=>{
      const url=storyUrl(a);
      const cat=db.categories.find(x=>x.id===a.categoryId)||c;
      const date=a.publishedAt||a.createdAt||'';
      return `<article class="short-timeline-item" data-cat-tone="${e(categoryTone(cat))}">
        <div class="short-timeline-rail" aria-hidden="true"><span class="short-timeline-no">${String(i+1).padStart(2,'0')}</span></div>
        <div class="short-timeline-content">
          <div class="short-timeline-meta"><span class="short-timeline-cat">${e(cat.name||'خبر')}</span><span class="short-timeline-views">◉ ${Number(a.views||0).toLocaleString('fa-IR')}</span></div>
          <h3><a href="${url}">${e(a.title)}</a></h3>
          <div class="short-timeline-bottom">${newsDate(date)}<a class="short-timeline-link" href="${url}">مشاهده خبر ←</a></div>
        </div>
      </article>`;
    }).join('');
    return shell('خبر کوتاه | نبض ساردو',`${header(db)}<main class="wrap page short-timeline-page"><div class="short-timeline-page-head"><span class="eyebrow">SHORT NEWS / LIVE FEED</span><h1>خبر کوتاه</h1><p>آخرین خبرهای کوتاه نبض ساردو به ترتیب زمان انتشار</p></div><div class="short-timeline-shell"><div class="short-timeline-list">${rows||'<div class="empty">هنوز خبر کوتاهی منتشر نشده است.</div>'}</div></div></main>${footer()}`,'خبرهای کوتاه نبض ساردو',{canonical:'/category/'+encodeURIComponent(c.id),robots:list.length?'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1':'noindex,follow'});
  }
  return shell('اخبار '+c.name+' | ساردوئیه و جنوب کرمان | نبض ساردو',`${header(db)}<main class="wrap page category-page" data-cat-tone="${e(categoryTone(c))}"><span class="eyebrow">CATEGORY</span><h1>${e(c.name)}</h1><div class="grid news">${list.map(a=>card(a,db)).join('')||'<div class="empty">هنوز خبری در این دسته منتشر نشده است.</div>'}</div></main>${footer()}`,'اخبار '+c.name+' در نبض ساردو',{canonical:'/category/'+encodeURIComponent(c.id),robots:list.length?'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1':'noindex,follow'});
}
function followup(db){
  const list=[...(db.followups||[])].sort((a,b)=>new Date(b.updatedAt||b.createdAt||0)-new Date(a.updatedAt||a.createdAt||0));
  const statusLabel={announced:'اعلام شد',started:'شروع شد',progress:'در حال اجرا',completed:'تکمیل شد',paused:'متوقف / در انتظار',closed:'مختومه'};
  const rows=list.map(x=>'<article class="panel"><span class="eyebrow">'+e(statusLabel[String(x.status||'announced')]||'پیگیری')+'</span><h2>'+e(x.title||'پرونده پیگیری')+'</h2>'+(x.promise?'<p>'+e(x.promise)+'</p>':'')+'<small>'+[x.promisor,x.location,x.deadline?'موعد: '+x.deadline:''].filter(Boolean).map(e).join(' · ')+'</small>'+(x.note?'<p><b>آخرین وضعیت:</b> '+e(x.note)+'</p>':'')+(x.sourceUrl?'<p><a class="more" href="'+e(x.sourceUrl)+'">'+e(x.sourceLabel||'مشاهده منبع')+' ←</a></p>':'')+'</article>').join('');
  const body=header(db)+'<main class="wrap page"><span class="eyebrow">FOLLOW-UP DESK / NABEZ SARDO</span><h1>پیگیری تا نتیجه</h1><p class="lead">پرونده‌های عمومی که نبض ساردو وضعیت آن‌ها را از زمان اعلام تا نتیجه دنبال می‌کند.</p><div class="grid info">'+(rows||'<div class="empty">هنوز پرونده عمومی برای پیگیری ثبت نشده است.</div>')+'</div></main>'+footer();
  return shell('پیگیری تا نتیجه | نبض ساردو',body,'پیگیری وعده‌ها، پروژه‌ها و مسائل عمومی ساردوئیه و جنوب کرمان تا رسیدن به نتیجه',{canonical:'/follow-up',robots:list.length?'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1':'noindex,follow'});
}
function editorialPolicy(db){const body=header(db)+'<main class="wrap page"><span class="eyebrow">EDITORIAL STANDARDS / NABEZ SARDO</span><h1>سیاست تحریریه و اصلاحات نبض ساردو</h1><p class="lead">نبض ساردو برای انتشار خبرهای محلی بر دقت، راستی‌آزمایی، ذکر نویسنده یا تحریریه، زمان انتشار، اصلاح خطا و شفافیت درباره منبع و تصویر خبر تأکید دارد.</p><div class="grid info"><div class="panel"><h3>دقت و راستی‌آزمایی</h3><p>خبرها پیش از انتشار بر اساس اطلاعات در دسترس، منابع رسمی، گزارش میدانی یا اطلاعات قابل بررسی تنظیم می‌شوند.</p></div><div class="panel"><h3>منبع و نویسنده</h3><p>نویسنده یا تحریریه، زمان انتشار و محل خبر در صفحه خبر نمایش داده می‌شود و در صورت اتکا به منبع بیرونی، منبع باید برای خواننده روشن باشد.</p></div><div class="panel"><h3>اصلاح خبر</h3><p>خطاهای معنادار پس از بررسی اصلاح یا تکمیل می‌شوند و خبر قدیمی صرفاً برای تازه جلوه دادن، به‌روزرسانی مصنوعی نمی‌شود.</p></div><div class="panel"><h3>عکس و کاور خودکار</h3><p>عکس واقعی خبر در اولویت است. کاور گرافیکی خودکار فقط تصویر معرفی خبر است و عکس واقعی رویداد محسوب نمی‌شود.</p></div><div class="panel"><h3>خبر مردمی</h3><p>ارسال متن، عکس یا ویدئو توسط کاربر به معنی تأیید یا انتشار خودکار نیست و محتوای ارسالی نیازمند بررسی تحریریه است.</p></div><div class="panel"><h3>ارتباط با تحریریه</h3><p>برای اعلام خطا، درخواست اصلاح یا ارائه اطلاعات تکمیلی از <a href="/contact">صفحه تماس با ما</a> استفاده کنید.</p></div></div></main>'+footer();return shell('سیاست تحریریه و اصلاحات | نبض ساردو',body,'اصول تحریریه، راستی‌آزمایی، اصلاح خبر، منابع و سیاست استفاده از تصاویر در نبض ساردو',{canonical:'/editorial-policy',schema:{'@context':'https://schema.org','@type':'WebPage',name:'سیاست تحریریه و اصلاحات نبض ساردو',url:absoluteUrl('/editorial-policy'),isPartOf:{'@type':'WebSite',name:'نبض ساردو',url:PUBLIC_BASE+'/'}}});}
function simple(db,type,ok=false,uploadError=''){let title='',content='';if(type==='about'){title='درباره نبض ساردو';content=`<p class="lead">نبض ساردو یک رسانه محلی برای پوشش دقیق، سریع و مسئولانه خبرهای ساردو، ساردوئیه، جیرفت، عنبرآباد، کهنوج و جنوب کرمان است.</p><div class="grid info"><div class="panel"><h3>ماموریت</h3><p>انتشار خبر محلی قابل اتکا، بازتاب مسائل واقعی مردم و حفظ هویت منطقه در قالب خبر، گزارش، عکس و ویدئو.</p></div><div class="panel"><h3>تحریریه و مسئولیت انتشار</h3><p>خبرها با نام نویسنده یا «تحریریه نبض ساردو» منتشر می‌شوند و زمان انتشار و محل خبر در صفحه هر خبر قابل مشاهده است.</p></div><div class="panel"><h3>استانداردهای تحریریه</h3><p>دقت، راستی‌آزمایی، تفکیک خبر از ادعا، اصلاح خطا و شفافیت درباره منبع و نوع تصویر از اصول کار تحریریه است. <a href="/editorial-policy">سیاست تحریریه و اصلاحات را بخوانید.</a></p></div><div class="panel"><h3>تمرکز منطقه‌ای</h3><p>ساردو و ساردوئیه، جیرفت، عنبرآباد، کهنوج و دیگر نقاط جنوب کرمان.</p></div><div class="panel"><h3>اصلاح خبر</h3><p>برای اعلام خطا یا ارائه اطلاعات تکمیلی می‌توانید از صفحه تماس استفاده کنید تا موضوع توسط تحریریه بررسی شود.</p></div><div class="panel"><h3>ارتباط رسمی</h3><p>راه ارتباط رسمی با تحریریه از طریق <a href="/contact">صفحه تماس با ما</a> و فرم ثبت پیام سایت است.</p></div></div>`;}else if(type==='contact'){title='تماس با ما';content=`<p class="lead">این صفحه راه ارتباط رسمی با تحریریه نبض ساردو برای ارسال توضیح، اعلام خطا، درخواست اصلاح یا پیگیری موضوعات خبری است.</p>${ok?'<div class="success">پیام شما ثبت شد.</div>':''}<form class="form" method="post"><label>نام<input name="name" required></label><label>راه ارتباطی<input name="contact" required></label><label>موضوع<input name="subject" required></label><label>پیام<textarea name="message" required></textarea></label><button class="btn primary">ارسال پیام</button></form>`;}else{title='ارسال خبر مردمی';content=`<div class="send-news-gift-box gift-card-box"><span>هدیه خبر مردمی</span><b>در صورت تمایل، شماره کارت خود را وارد کنید تا اگر خبر شما مشمول هدیه شد، مبلغ برایتان واریز شود.</b></div>${ok?'<div class="success">خبر شما همراه فایل برای تحریریه ثبت شد.</div>':''}${uploadError==='large'?'<div class="error">حجم فایل بیشتر از ۵۰ مگابایت است.</div>':uploadError==='type'?'<div class="error">فرمت فایل پشتیبانی نمی‌شود. عکس یا ویدئوی MP4/WebM/MOV انتخاب کنید.</div>':''}<form class="form citizen-form" method="post" enctype="multipart/form-data"><label>نام<input name="name" required></label><label>شماره تماس<input name="phone" required></label><label>محل رویداد<input name="location" required></label><label>عنوان خبر<input name="headline" required></label><label>شرح خبر<textarea name="details" required></textarea></label><label class="citizen-upload">عکس یا ویدئوی خبر<input type="file" name="mediaFile" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" required><small>فایل را مستقیم از گوشی انتخاب و آپلود کنید؛ حداکثر حجم ۵۰ مگابایت.</small></label><label class="reward-card-input">شماره کارت برای دریافت هدیه <small>اختیاری — فقط برای واریز هدیه احتمالی استفاده می‌شود.</small><input name="rewardCard" inputmode="numeric" autocomplete="off" maxlength="24" placeholder="مثلاً 6037 99xx xxxx xxxx"></label><button class="btn primary">ارسال برای تحریریه</button></form>`;}const canonical=type==='about'?'/about':type==='contact'?'/contact':'/send-news';return shell(title+' | نبض ساردو',`${header(db)}<main class="wrap page"><span class="eyebrow">NABEZ SARDO</span><h1>${title}</h1>${content}</main>${footer()}`,title+' | نبض ساردو',{canonical,robots:type==='send-news'?'noindex,follow':undefined});}
module.exports={home,archive,briefs,article,category,simple,search,localHub,followup,editorialPolicy};
