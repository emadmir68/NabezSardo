...
function home(db){
 ...
 const highlightsMarkup=highlightNews.map((a,i)=>{
   ...
   return `<article class="front-brief" data-cat-tone="${e(categoryTone(c))}">
     <a class="front-brief-media" href="${url}" style="position:relative"><img src="${e(image)}" alt="${e(a.title)}" loading="lazy" decoding="async" fetchpriority="low">${a.videoUrl?'<span style="position:absolute;left:7px;bottom:7px;padding:4px 7px;border-radius:999px;background:rgba(7,10,15,.82);border:1px solid rgba(232,190,109,.3);color:#f0c979;font-size:8px;font-weight:900">▶ ویدئو</span>':''}</a>
