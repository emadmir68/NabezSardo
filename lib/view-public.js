...
const LOCAL_HUBS={
  sardouiyeh:{name:'ساردوئیه',en:'Sardouiyeh',terms:['ساردوئیه','ساردو'],description:'آخرین اخبار ساردوئیه؛ رویدادهای محلی، اجتماعی، کشاورزی، گردشگری و مطالبات مردم.'},
  jiroft:{name:'جیرفت',en:'Jiroft',terms:['جیرفت'],description:'آخرین اخبار جیرفت؛ تازه‌ترین رویدادهای شهرستان جیرفت و جنوب کرمان.'},
  'south-kerman':{name:'جنوب کرمان',en:'South Kerman',terms:['جنوب کرمان','کرمان جنوبی'],description:'آخرین اخبار جنوب کرمان؛ پوشش خبرهای شهرستان‌ها، اقتصاد، کشاورزی، جامعه و رویدادهای منطقه.'}
};
function localHub(db,key){
  ...
  return shell('اخبار '+hub.name+' امروز | نبض ساردو',body,hub.description,{canonical,schema});
}
function home(db){
 ...
 <section class="wrap citizen"><div class="citizen-copy"><span class="eyebrow">CITIZEN NEWSROOM</span><h2>شما هم خبرنگار نبض ساردو باشید</h2><p>خبر، عکس، ویدئو یا سوژه محلی را برای تحریریه ارسال کنید.</p></div><a class="btn primary" href="/send-news">خبرتو بفرست و هدیه بگیر</a></section></main>${footer()}`, 'نبض ساردو؛ تازه‌ترین اخبار ساردوئیه، جیرفت و جنوب کرمان',{canonical:'/',schema:siteSchema(),preloadImage:'https://d2ol7oe51mr4n9.cloudfront.net/user_32MkkZvMhQKFjHMKrhiL97UtBqA/03895380-5279-4699-9bf8-671489cf2a89.jpg?v=four-seasons-final-1',preconnect:'https://d2ol7oe51mr4n9.cloudfront.net'});}


function editionPreview(db){
  const all=published(db);
  const regular=all.filter(function(a){return a.categoryId!=='short-news'});
  const shorts=all.filter(function(a){return a.categoryId==='short-news'}).slice(0,6);
  const lead=regular.find(function(a){return a.featured})||regular[0]||null;
  const side=regular.filter(function(a){return !lead||a.id!==lead.id}).slice(0,4);
  const followup=followupSorted(db)[0]||null;
  const categoryOf=function(a){return db.categories.find(function(x){return x.id===(a&&a.categoryId)})};
  const storyUrl=function(a){return '/news/'+encodeURIComponent(a.slug)};
  let today='';
  try{today=new Intl.DateTimeFormat('fa-IR-u-ca-persian',{weekday:'long',year:'numeric',month:'long',day:'numeric',timeZone:'Asia/Tehran'}).format(new Date())}catch{}
  const editionNo=(new Date().getFullYear())+' / '+Math.max(1,Math.ceil((Date.now()-new Date(new Date().getFullYear(),0,1).getTime()+1)/86400000)).toLocaleString('fa-IR');

  let leadBlock='<div class="edition-empty">هنوز خبری برای ساخت نسخه امروز منتشر نشده است.</div>';
  if(lead){
    const cat=categoryOf(lead),url=storyUrl(lead),image=lead.image||'/assets/placeholder.svg';
    leadBlock='<article class="edition-lead">'+
      '<a class="edition-lead-media" href="'+url+'"><img src="'+e(image)+'" alt="'+e(lead.title)+'" decoding="async" fetchpriority="high"></a>'+
      '<div class="edition-lead-copy"><div class="edition-rule"></div><span class="edition-kicker">'+e((cat&&cat.name)||'خبر اول')+'</span>'+
      '<h1><a href="'+url+'">'+e(lead.title)+'</a></h1><p>'+e(compactText(lead.lead||lead.body||'',260))+'</p>'+
      '<div class="edition-lead-meta"><span>'+e(lead.location||'ساردوئیه')+'</span><span>◉ '+Number(lead.views||0).toLocaleString('fa-IR')+'</span><a href="'+url+'">ادامه خبر ←</a></div></div></article>';
  }

  const sideBlock=side.map(function(a,i){
    const cat=categoryOf(a),url=storyUrl(a);
    return '<article class="edition-side-story"><span class="edition-side-no">'+String(i+1).padStart(2,'0')+'</span><div>'+
      '<span>'+e((cat&&cat.name)||'خبر')+'</span><h2><a href="'+url+'">'+e(a.title)+'</a></h2><p>'+e(compactText(a.lead||a.body||'',105))+'</p></div></article>';
  }).join('');

  const shortsBlock=shorts.length?'<section class="edition-briefs"><div class="edition-section-head"><span>SHORT DESK</span><h2>خبرهای کوتاه امروز</h2></div><div class="edition-briefs-grid">'+
    shorts.map(function(a,i){return '<a href="'+storyUrl(a)+'"><b>'+String(i+1).padStart(2,'0')+'</b><span>'+e(a.title)+'</span></a>'}).join('')+
    '</div></section>':'';

  let followupBlock='';
  if(followup){
    const meta=followupStatusMeta(followup);
    followupBlock='<section class="edition-followup" data-fu-status="'+e(meta.key)+'"><div class="edition-section-head"><span>FOLLOW-UP DESK</span><h2>قول دادند؛ چی شد؟</h2></div>'+
      '<div class="edition-followup-grid"><div><span class="edition-followup-state">'+e(meta.label)+'</span><h3>'+e(followup.title||'پرونده پیگیری')+'</h3>'+
      '<p>'+e(compactText(followup.promise||followup.note||'',220))+'</p></div><div class="edition-followup-meta">'+
      (followup.promisor?'<span><small>اعلام‌کننده</small><b>'+e(followup.promisor)+'</b></span>':'')+
      (followup.location?'<span><small>محدوده</small><b>'+e(followup.location)+'</b></span>':'')+
      (followup.deadline?'<span><small>موعد</small><b>'+e(faDateOnly(followup.deadline))+'</b></span>':'')+
      '<a href="/follow-up">مشاهده پرونده ←</a></div></div></section>';
  }

  const body='<main class="edition-page">'+
    '<div class="edition-topbar"><a href="/">← بازگشت به سایت</a><span>پیش‌نمایش نسخه روزانه</span><button type="button" data-edition-share>اشتراک نسخه</button></div>'+
    '<div class="edition-paper"><header class="edition-masthead"><div class="edition-date"><span>نسخه روزانه</span><b>'+e(today)+'</b></div>'+
    '<div class="edition-brand"><small>THE DAILY EDITION OF</small><strong>نبض ساردو</strong><em>NABEZ SARDO</em></div>'+
    '<div class="edition-number"><span>شماره</span><b>'+e(editionNo)+'</b></div></header>'+
    '<div class="edition-divider"><i></i><span>خبر دقیق · روایت انسانی · هویت منطقه</span><i></i></div>'+
    '<section class="edition-hero-grid">'+leadBlock+'<aside class="edition-side">'+(sideBlock||'<div class="edition-empty">خبر دیگری برای این نسخه وجود ندارد.</div>')+'</aside></section>'+
    shortsBlock+followupBlock+
    '<footer class="edition-footer"><div><b>نبض ساردو</b><span>ساردوئیه · جیرفت · جنوب کرمان</span></div><a href="/">nabzesardo.ir</a><span>نسخه آزمایشی صفحه اول امروز</span></footer></div></main>';
  return shell('نسخه امروز نبض ساردو | پیش‌نمایش',body,'پیش‌نمایش نسخه روزانه نبض ساردو',{canonical:'/edition-preview',robots:'noindex,nofollow'});
}
...
function article(db,a){
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(shareUrl)}" data-article-canonical="${e(articleUrl)}" data-article-image="${e(a.image||'')}" data-article-lead="${e(shareLead)}">
      ...
      ${a.image?`<figure class="article-cover-wrap"><div class="cover-frame ${imageOrientation(a)}" data-adaptive-media style="--image-ratio:${imageRatio(a)}"><img class="cover" src="${e(a.image)}" alt="${e(a.title)}" loading="eager" decoding="async" fetchpriority="high"></div><figcaption><span class="lang-fa">تصویر خبر · نبض ساردو</span><span class="lang-en">News image · Nabez Sardo</span></figcaption></figure>`:''}
...
module.exports={home,archive,briefs,article,category,simple,search,localHub,nabez60,followup,editionPreview};