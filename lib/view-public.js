...
// Experimental, isolated homepage feature. Set to false for an instant rollback.
const LIVE_REGION_MAP_ENABLED=true;
const REGION_PLACES=[
  {id:'sardouiyeh',name:'ساردوئیه',nameEn:'Sardouiyeh',lat:29.1500,lng:57.5500,keys:['ساردوئیه','ساردویه','درب بهشت']},
  {id:'jiroft',name:'جیرفت',nameEn:'Jiroft',lat:28.6751,lng:57.7372,keys:['جیرفت']},
  {id:'anbarabad',name:'عنبرآباد',nameEn:'Anbarabad',lat:28.4783,lng:57.8414,keys:['عنبرآباد','عنبر اباد']},
  {id:'kahnuj',name:'کهنوج',nameEn:'Kahnuj',lat:27.9514,lng:57.6994,keys:['کهنوج']},
  {id:'rudbar',name:'رودبار جنوب',nameEn:'Rudbar-e Jonubi',lat:27.5751,lng:58.9353,keys:['رودبار جنوب','رودبارجنوب','رودبار']},
  {id:'manujan',name:'منوجان',nameEn:'Manujan',lat:27.4060,lng:57.5013,keys:['منوجان']},
  {id:'qalehganj',name:'قلعه‌گنج',nameEn:'Qaleh Ganj',lat:27.5236,lng:57.8811,keys:['قلعه گنج','قلعه‌گنج']},
  {id:'faryab',name:'فاریاب',nameEn:'Faryab',lat:28.0983,lng:57.2311,keys:['فاریاب']}
];
function newsRegionPlace(a){
  const hay=[a?.location,a?.title,a?.lead,a?.body].filter(Boolean).join(' ').replace(/ي/g,'ی').replace(/ك/g,'ک');
  return REGION_PLACES.find(p=>p.keys.some(k=>hay.includes(k)))||null;
}
function siteSchema(){
  return [
...
function home(db){
 ...
 const regionMapGroups=REGION_PLACES.map(place=>{
   const items=regularNews.filter(a=>newsRegionPlace(a)?.id===place.id).slice(0,6);
   return {...place,items};
 });
 const mappedNews=regularNews.map(a=>({a,place:newsRegionPlace(a)})).filter(x=>x.place).slice(0,6);
 const activeRegionCount=regionMapGroups.filter(g=>g.items.length).length;
 const regionMapPointsMarkup=regionMapGroups.map(g=>{
   const latestItem=g.items[0]||null;
   return `<span hidden data-map-place data-place-id="${e(g.id)}" data-name="${e(g.name)}" data-name-en="${e(g.nameEn)}" data-lat="${g.lat}" data-lng="${g.lng}" data-count="${g.items.length}" data-title="${latestItem?e(latestItem.title):''}" data-title-en="${latestItem?e(latestItem.titleEn||latestItem.title):''}" data-url="${latestItem?e(storyUrl(latestItem)):''}"></span>`;
 }).join('');
 const regionMapFeed=mappedNews.map(({a,place},i)=>`<a class="region-map-feed-item" href="${storyUrl(a)}">
   <span class="region-map-feed-rank">${String(i+1).padStart(2,'0')}</span>
   <span class="region-map-feed-copy"><small>${e(place.name)}</small><b><span class="lang-fa">${e(a.title)}</span><span class="lang-en">${e(a.titleEn||a.title)}</span></b></span>
   <span class="region-map-feed-arrow" aria-hidden="true">←</span>
 </a>`).join('');
 ...
 ${LIVE_REGION_MAP_ENABLED?`<section class="wrap live-region-section" aria-label="نبض زنده منطقه" data-live-region>
   <div class="live-region-shell">
     <div class="live-region-head">
       <div>
         <span class="live-region-kicker"><i aria-hidden="true"></i><span>LIVE REGION / MAP DESK</span></span>
         <h2><span class="lang-fa">نبض زنده منطقه</span><span class="lang-en">Live Region Pulse</span></h2>
         <p><span class="lang-fa">خبرهای جنوب کرمان را روی نقشه ببینید؛ هر نقطه فعال، تازه‌ترین خبر همان منطقه را باز می‌کند.</span><span class="lang-en">Explore South Kerman news on the map. Active points open the latest story from each area.</span></p>
       </div>
       <div class="live-region-status">
         <span class="live-region-live"><i></i> LIVE</span>
         <small><b>${activeRegionCount.toLocaleString('fa-IR')}</b> <span class="lang-fa">منطقه فعال</span><span class="lang-en">active areas</span></small>
       </div>
     </div>
     <div class="live-region-grid">
       <div class="live-region-map-card">
         <div class="live-region-map" data-news-map aria-label="نقشه تعاملی خبرهای جنوب کرمان">
           ${regionMapPointsMarkup}
           <div class="live-region-map-loading" data-map-status><span></span><b class="lang-fa">در حال بارگذاری نقشه زنده…</b><b class="lang-en">Loading live map…</b></div>
         </div>
         <div class="live-region-map-hud" aria-hidden="true"><span>NABEZ / GEO DESK</span><b>28°N · 58°E</b></div>
       </div>
       <aside class="live-region-feed">
         <div class="live-region-feed-head">
           <span class="lang-fa">آخرین خبرهای مکانی</span><span class="lang-en">Latest mapped stories</span>
           <i></i>
         </div>
         <div class="live-region-feed-list">
           ${regionMapFeed||`<div class="live-region-empty"><span class="lang-fa">با انتشار خبرهای دارای نام شهر یا منطقه، این بخش به‌صورت خودکار فعال می‌شود.</span><span class="lang-en">This feed activates automatically as location-based stories are published.</span></div>`}
         </div>
       </aside>
     </div>
   </div>
...
function article(db,a){
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(articleUrl)}">
...
module.exports={home,archive,briefs,article,category,simple,search};