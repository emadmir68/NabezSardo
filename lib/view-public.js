...
function home(db){
 ...
 const heroSpotlightMarkup=leadStory?(()=>{
   const c=categoryOf(leadStory),url=storyUrl(leadStory);
   return `<a class="ns-hero-final__spotlight" href="${url}">
     <span class="ns-hero-final__spotlight-live"><i></i><span class="lang-fa">خبر ویژه امروز</span><span class="lang-en">TODAY'S FEATURED STORY</span></span>
     <strong><span class="lang-fa">${e(leadStory.title)}</span><span class="lang-en">${e(leadStory.titleEn||leadStory.title)}</span></strong>
     <span class="ns-hero-final__spotlight-meta"><b><span class="lang-fa">${e(c?.name||'خبر')}</span><span class="lang-en">${e(catEn(c?.name||'News'))}</span></b><span>◉ ${Number(leadStory.views||0).toLocaleString('fa-IR')}</span><time class="news-date" data-news-date="${e(leadStory.publishedAt||leadStory.createdAt||'')}"></time><em><span class="lang-fa">مشاهده خبر ←</span><span class="lang-en">Open story →</span></em></span>
   </a>`;
 })():'';
