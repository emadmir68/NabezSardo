...
function home(db){
 ...
 const todayPulse=allNews.slice(0,5);
 ...
 const breakingStamp=db.settings?.breakingUpdatedAt||'';
 ...
 <section class="breaking ${liveDeskOn?'live-desk-on':'live-desk-off'}" data-live-desk="${liveDeskOn?'on':'off'}"><div class="wrap breaking-row">
   ...
   <i class="breaking-live ${liveDeskOn?'':'breaking-live-off'}"><span class="breaking-live-dot" aria-hidden="true"></span><span>${liveDeskOn?'LIVE DESK':'DESK OFF'}</span>${breakingStamp?'<small><span class="lang-fa">به‌روزرسانی </span><span class="lang-en">Updated </span><time class="news-date" data-news-date="'+e(breakingStamp)+'"></time></small>':''}</i>
 </div></section>
...
function briefs(db){
  const list=published(db).filter(a=>a.shortNews===true);
  const rows=list.map((a,i)=>{
    const cat=db.categories.find(x=>x.id===a.categoryId);
    const url='/news/'+encodeURIComponent(a.slug);
    const lead=compactText(a.lead||a.body||'',150);
    return `<article class="brief-row" data-cat-tone="${e(categoryTone(cat))}">
      <div class="brief-time"><span class="brief-live-dot" aria-hidden="true"></span><time class="news-date" data-news-date="${e(a.publishedAt||a.createdAt||'')}"></time></div>
      <div class="brief-copy">
        <div class="brief-kicker"><span>${String(i+1).padStart(2,'0')}</span><a href="/category/${encodeURIComponent(a.categoryId||'')}">${e(cat?.name||'خبر')}</a></div>
        <h2><a href="${url}">${e(a.title)}</a></h2>
        ${lead?`<p>${e(lead)}</p>`:''}
      </div>
      <a class="brief-open" href="${url}" aria-label="باز کردن خبر">←</a>
    </article>`;
  }).join('');
  return shell('خبر کوتاه | نبض ساردو',`${header(db)}<main class="wrap page brief-page">
    <section class="brief-hero">
      <div><span class="eyebrow">SHORT NEWS / LIVE DESK</span><h1><span class="lang-fa">خبر کوتاه</span><span class="lang-en">Short News</span></h1><p><span class="lang-fa">مهم‌ترین خبرها در یک یا دو خط؛ سریع، خلاصه و مستقیم.</span><span class="lang-en">Key updates in one or two lines — fast and direct.</span></p></div>
      <div class="brief-hero-badge"><i></i><span class="lang-fa">به‌روزرسانی پیوسته</span><span class="lang-en">Live updates</span></div>
    </section>
    <section class="brief-feed">${rows||'<div class="brief-empty"><b>فعلاً خبر کوتاهی منتشر نشده است.</b><span>از پنل تحریریه گزینه «نمایش در خبر کوتاه» را برای خبر موردنظر فعال کنید.</span></div>'}</section>
  </main>${footer()}`,'خبرهای کوتاه و فوری نبض ساردو؛ مهم‌ترین رویدادها در یک یا دو خط',{canonical:'/briefs'});
}
