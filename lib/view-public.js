...
function localHub(db,key){
  ...
  return shell('اخبار '+hub.name+' امروز | آخرین خبرهای جنوب کرمان | نبض ساردو',body,hub.description+' | نبض ساردو؛ پوشش خبری محلی جنوب کرمان.',{canonical,schema,robots:list.length?'index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1':'noindex,follow'});
}
function home(db){
 ...
 const shortNewsLifetimeMs=60*60*1000;
 const shortNewsNowMs=Date.now();
 const shortNews=allNews
   .filter(a=>{
     if(a.categoryId!=='short-news')return false;
     const publishedMs=new Date(a.publishedAt||a.createdAt||0).getTime();
     return Number.isFinite(publishedMs)&&publishedMs<=shortNewsNowMs&&(shortNewsNowMs-publishedMs)<shortNewsLifetimeMs;
   })
 ...
 const shortHeroMarkup=shortNews.length?`<aside class="hero-short-story-stack" data-short-story-stack aria-label="خبر کوتاه یک‌ساعته">
   <div class="hero-short-story-head">
     <span class="hero-short-story-label"><i aria-hidden="true"></i><span class="lang-fa">خبر کوتاه</span><span class="lang-en">SHORT NEWS</span></span>
     <span class="hero-short-story-brand"><span class="lang-fa">نبض ساردو · زنده</span><span class="lang-en">NABEZ SARDO · LIVE</span></span>
   </div>
   <div class="hero-short-story-viewport">
     ${shortNews.map((a,i)=>{
       const url=storyUrl(a);
       const publishedMs=new Date(a.publishedAt||a.createdAt||0).getTime();
       const expiresAt=publishedMs+shortNewsLifetimeMs;
       return `<a class="hero-short-story ${i===0?'is-active':''}" data-short-story data-expires="${expiresAt}" href="${url}" aria-hidden="${i===0?'false':'true'}">
         <span class="hero-short-story-copy">
           <strong class="hero-short-story-text"><span class="lang-fa">${e(a.title)}</span><span class="lang-en">${e(a.titleEn||a.title)}</span></strong>
           <small><span class="lang-fa">مشاهده خبر ←</span><span class="lang-en">Open story →</span></small>
         </span>
       </a>`;
     }).join('')}
   </div>
   <div class="hero-short-story-foot">
     <span class="hero-short-story-progress" aria-hidden="true"><i data-short-story-progress></i></span>
     <span class="hero-short-story-counter" data-short-story-counter aria-live="polite"></span>
   </div>
 </aside>`:'';
