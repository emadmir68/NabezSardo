...
const LOCAL_HUBS={
  sardouiyeh:{name:'ساردوئیه',en:'Sardouiyeh',terms:['ساردوئیه','ساردویه','ساردو'],description:'آخرین اخبار ساردوئیه؛ رویدادهای محلی، اجتماعی، کشاورزی، گردشگری و مطالبات مردم.'},
  jiroft:{name:'جیرفت',en:'Jiroft',terms:['جیرفت'],description:'آخرین اخبار جیرفت؛ تازه‌ترین رویدادهای شهرستان جیرفت و جنوب کرمان.'},
  anbarabad:{name:'عنبرآباد',en:'Anbarabad',terms:['عنبرآباد','عنبر آباد','عنبر اباد'],description:'آخرین اخبار عنبرآباد؛ رویدادهای شهری و روستایی، کشاورزی، اقتصاد، جامعه و مطالبات مردم.'},
  kahnuj:{name:'کهنوج',en:'Kahnuj',terms:['کهنوج'],description:'آخرین اخبار کهنوج؛ تازه‌ترین رویدادهای شهرستان کهنوج و جنوب کرمان.'},
  'south-kerman':{name:'جنوب کرمان',en:'South Kerman',terms:['جنوب کرمان','کرمان جنوبی','ساردوئیه','ساردویه','جیرفت','عنبرآباد','عنبر آباد','کهنوج'],description:'آخرین اخبار جنوب کرمان؛ پوشش خبرهای شهرستان‌ها، اقتصاد، کشاورزی، جامعه و رویدادهای منطقه.'}
};
function localHub(db,key){
  const hub=LOCAL_HUBS[key];if(!hub)return null;
  const list=published(db).filter(a=>{const hay=[a.location,a.title,a.lead,a.body].filter(Boolean).join(' ');return hub.terms.some(t=>hay.includes(t));});
  ...
  const body=header(db)+'<main class="wrap page category-page local-hub-page"><span class="eyebrow">LOCAL NEWS / '+e(hub.en)+'</span><h1>آخرین اخبار '+e(hub.name)+'</h1><p class="lead">'+e(hub.description)+'</p><div class="grid news">'+(list.map(a=>card(a,db)).join('')||'<div class="empty">هنوز خبری برای این منطقه منتشر نشده است.</div>')+'</div></main>'+footer();
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(shareUrl)}" data-article-canonical="${e(articleUrl)}" data-article-image="${e(a.image||'')}" data-article-lead="${e(shareLead)}">
      <header class="article-head">
        ...
        <div class="article-trustline"><span><b><span class="lang-fa">نویسنده:</span><span class="lang-en">By:</span></b> ${author}</span><span aria-hidden="true">•</span><span><b><span class="lang-fa">انتشار:</span><span class="lang-en">Published:</span></b> <time class="news-date" data-news-date="${e(a.publishedAt||a.createdAt||'')}"></time></span></div>
    ...
    const rows=list.map((a,i)=>{
      ...
      return `<article class="short-timeline-item" data-cat-tone="${e(categoryTone(cat))}">
        ...
        <div class="short-timeline-content">
          ...
          <div class="short-timeline-bottom"><time class="news-date" data-news-date="${e(date)}"></time><a class="short-timeline-link" href="${url}">مشاهده خبر ←</a></div>
        </div>
      </article>`;
    }).join('');
