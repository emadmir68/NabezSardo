...
const {e,shell,header,footer,card,oldBody,imageRatio,imageOrientation,absoluteUrl,shortArticleCode,PUBLIC_BASE,catEn,categoryTone}=require('./view-common');
...
function articleSchema(a,c){
  ...
  const schemaImage=a.socialImage||a.image||'';
  if(schemaImage)news.image=[absoluteUrl(schemaImage)];
  if(a.videoUrl)news.video={
    ...
    thumbnailUrl:schemaImage?[absoluteUrl(schemaImage)]:undefined
  };
...
function home(db){
 ...
 const categoryTerminalMarkup=db.categories.slice(0,9).map((c,i)=>{
...
function article(db,a){
  ...
  const shareUrl='/news/'+shortArticleCode(a.id);
  const previewImage=a.socialImage||a.image||'/assets/header-mosque-final.jpg';
  ...
  <main class="wrap article article-premium" data-cat-tone="${tone}">
    ...
    <article class="article-shell" data-article-root data-article-title="${e(a.title)}" data-article-url="${e(shareUrl)}" data-article-canonical="${e(articleUrl)}" data-article-image="${e(previewImage)}" data-article-image-kind="${e(!a.image?'none':/auto-cover/i.test(String(a.image))?'auto':'real')}" data-article-video="${e(a.videoUrl||'')}" data-article-video-type="${e(a.videoType||'video/mp4')}" data-article-category-id="${e(a.categoryId||'')}" data-article-lead="${e(shareLead)}">
      <header class="article-head">
        ...
        <div class="article-actions">
          <button class="article-action article-share" type="button" data-article-share><span class="article-action-icon" aria-hidden="true">↗</span><span class="lang-fa">اشتراک لینک</span><span class="lang-en">Share link</span></button>
          <button class="article-action article-share-image" type="button" data-article-share-image><span class="article-action-icon" aria-hidden="true">▣</span><span data-share-image-label><span class="lang-fa">اشتراک با عکس</span><span class="lang-en">Share with image</span></span></button>
          ${a.videoUrl?`<button class="article-action article-share-video" type="button" data-article-share-video><span class="article-action-icon" aria-hidden="true">▶</span><span data-share-video-label><span class="lang-fa">اشتراک با فیلم</span><span class="lang-en">Share with video</span></span></button>`:''}
          ...
          <button class="article-action article-copy-message" type="button" data-article-copy-message><span class="article-action-icon" aria-hidden="true">✦</span><span data-copy-message-label><span class="lang-fa">کپی متن آماده</span><span class="lang-en">Copy ready message</span></span></button>
    ...
    modifiedTime:a.updatedAt||a.publishedAt||a.createdAt||'',
    section:c?.name||'خبر',
    schema:articleSchema(a,c)
  });
}
...
module.exports={home,archive,briefs,article,category,simple,search,localHub};