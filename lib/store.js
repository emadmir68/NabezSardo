...
function autoBackup(){
  ...
  const week=Math.ceil((((Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate())-first.getTime())/86400000)+first.getUTCDay()+1)/7);
  const key=String(now.getUTCFullYear())+'-W'+String(week).padStart(2,'0');
  const target=path.join(BACKUP_DIR,'auto-weekly-'+key+'.json');
