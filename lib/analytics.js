...
function empty(){return {version:1,startedAt:new Date().toISOString(),days:{}};}
function load(){
  ensureDir();
  try{
...
function isBot(ua=''){
  return /(bot|crawl|spider|slurp|bingpreview|facebookexternalhit|whatsapp|telegrambot|discordbot|preview|headless|lighthouse|pagespeed|uptime|monitor)/i.test(String(ua));
}
function isTrackablePath(p=''){
  ...
  return /^\/$|^\/all-news$|^\/search$|^\/about$|^\/contact$|^\/send-news$|^\/news\/|^\/category\//.test(p);
}
...
function ensureDay(data,key){
  const d=data.days[key]||(data.days[key]={pageViews:0,googleEntrances:0,externalEntrances:0,directEntrances:0,visitors:{},paths:{},lastAt:null});
...
function track(req,pathname){
  ...
  const data=load();
  const key=dayKey(),d=ensureDay(data,key);
  d.pageViews++;
  ...
  const internal=!host||host==='nabzesardo.ir'||host.endsWith('.nabzesardo.ir')||host.endsWith('.up.railway.app')||host.endsWith('.workers.dev');
...
function snapshot(){
  ...
  return {
    ...
    todayKey,
    today:{pageViews:today.pageViews,visitors:Object.keys(today.visitors).length,googleEntrances:today.googleEntrances,externalEntrances:today.externalEntrances,directEntrances:today.directEntrances},
