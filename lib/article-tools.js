...
const FEATURED_WIDTH=1600;
const FEATURED_HEIGHT=900;
...
async function saveFeaturedImage(file){
  ...
  await sharp(file.data,{animated:false})
    .rotate()
    .resize(FEATURED_WIDTH,FEATURED_HEIGHT,{fit:'cover',position:'attention'})
...
async function normalizeExistingFeaturedImages(){
  const db=load();
  let changed=false;
  for(const a of db.articles){
    if(!a.image||!String(a.image).startsWith('/uploads/'))continue;
    const oldName=path.basename(a.image);
    if(oldName.startsWith('featured-'))continue;
    const src=path.join(UPLOAD_DIR,oldName);
    ...
    try{
      await sharp(src,{animated:false})
        ...
        .webp({quality:88,smartSubsample:true})
        .toFile(target);
      a.image='/uploads/'+name;
      a.updatedAt=now();
      changed=true;
    }catch(err){
      console.error('featured image migration failed',a.id,err.message);
    }
  }
  if(changed)save(db);
  return changed;
...
async function articlePayload(db,f,files,current={},uniqueSlug){
  ...
  if(files.imageFile&&files.imageFile.data.length){
    const old=image;
    image=await saveFeaturedImage(files.imageFile);
    if(old&&old!==image)deleteUpload(old);
  }
  ...
  return {
    ...
    status,scheduledAt,featured:f.featured==='1',image,gallery:parseGallery(f.galleryJson,current.gallery||[]),
...
module.exports={saveImage,saveFeaturedImage,deleteUpload,normalizeExistingFeaturedImages,sanitizeRich,plainFromRich,parseGallery,parseTehranDate,articlePayload,dispatchAndPersist,schedulerTick,socialStatus:social.status};