...
async function articlePayload(db,f,files,current={},uniqueSlug){
  ...
  return {
    ...
    lead,bodyHtml,body:plainFromRich(bodyHtml),author:normalizeSingleLine(f.author||'تحریریه نبض ساردو',120),location:normalizeSingleLine(f.location||'',120),
...
async function backfillTypography(){
  ...
  for(const a of db.articles){
    ...
    const location=normalizeSingleLine(a.location||'',120);
...
async function dispatchAndPersist(articleId){
  ...
  a.shareKit=social.sharePackage({...a,categoryName:category.name||''});
  a.shareKitGeneratedAt=now();
  a.distribution=await social.dispatch(a);
  a.socialDispatchedAt=now();
  save(db);
  return {shareKit:a.shareKit,distribution:a.distribution};
}
async function schedulerTick(){
  ...
  save(db);
  for(const a of due){try{await module.exports.dispatchAndPersist(a.id,{retry:true})}catch(err){console.error('scheduled distribution',a.id,err);}}
}
