...
async function backfillFeaturedMetadata(){
  ...
  for(const a of db.articles){
    if(!a.image||a.imageWidth&&a.imageHeight&&a.imageRatio)continue;
    if(!String(a.image).startsWith('/uploads/'))continue;
    ...
    }catch(err){console.error('featured metadata backfill failed',a.id,err.message);}
  }
  if(changed)save(db);
  return changed;
}
...
module.exports={saveImage,saveFeaturedImage,deleteUpload,backfillFeaturedMetadata,sanitizeRich,plainFromRich,parseGallery,parseTehranDate,articlePayload,dispatchAndPersist,schedulerTick,socialStatus:social.status};