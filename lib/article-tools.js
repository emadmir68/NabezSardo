...
async function articlePayload(db,f,files,current={},uniqueSlug){
  ...
  return {
    ...
    status,scheduledAt,featured:f.featured==='1',...meta,gallery:parseGallery(f.galleryJson,current.gallery||[]),
...
async function backfillTypography(){
  ...
  for(const a of db.articles){
    ...
    if((a.lead||'')!==lead){a.lead=lead;changed=true;}
    if((a.author||'')!==author){a.author=author;changed=true;}
    if((a.location||'')!==location){a.location=location;changed=true;}
...
module.exports={saveImage,saveFeaturedImage,deleteUpload,backfillFeaturedMetadata,backfillTypography,sanitizeRich,normalizeRich,normalizeSingleLine,plainFromRich,parseGallery,parseTehranDate,articlePayload,dispatchAndPersist,schedulerTick,socialStatus:social.status};