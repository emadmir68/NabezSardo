...
async function rubika(a){
  ...
  if(data&&typeof data==='object'){
    const status=String(data.status||data.status_det||data.message||'').toLowerCase();
    if(status.includes('error')||status.includes('fail'))throw new Error('Rubika API rejected sendMessage');
  }
  return {status:'sent',at:new Date().toISOString(),mode:'text'};
}
async function testRubikaConnection(){
  const token=process.env.RUBIKA_BOT_TOKEN,chat=process.env.RUBIKA_CHAT_ID;
  if(!token||!chat)return {ok:false,reason:'not-configured'};
  const url='https://botapi.rubika.ir/v3/'+encodeURIComponent(token)+'/sendMessage';
  const res=await fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({chat_id:chat,text:'تست اتصال نبض ساردو به روبیکا'})});
  let data=null;try{data=await res.json()}catch{data=await res.text()}
...
module.exports={status,dispatch,articleUrl,testRubikaConnection};