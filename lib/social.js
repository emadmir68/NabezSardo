...
async function rubika(a){
  ...
  if(a.image&&BASE){
    try{
      ...
      await rubikaCall(token,'sendFile',{chat_id:chat,text:message.text,metadata:message.metadata,file_id:fileId});
      return {status:'sent',at:new Date().toISOString(),mode:'image'};
    }catch(err){
      console.warn('rubika image fallback',String(err.message||err));
    }
  }
