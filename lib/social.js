...
const CATEGORY_TAGS={
  ...
  'video':'ویدئو','short-news':'خبر_کوتاه','opinion':'یادداشت_و_مطالبه'
};
...
function contentLabels(a){
  if(String(a&&a.categoryId||'')==='opinion'){
    return {icon:'📣',summary:'خلاصه مطالبه',continueText:'متن کامل مطالبه در نبض ساردو'};
  }
  return {icon:'📰',summary:'خلاصه خبر',continueText:'ادامه خبر در نبض ساردو'};
}
function telegramText(a,source='telegram'){
  ...
  const labels=contentLabels(a);
  const lines=[
    '🟠 <b>نبض ساردو</b>',
    labels.icon+' <b>'+escHtml(title)+'</b>',
    lead?'🔹 <b>'+escHtml(labels.summary)+'</b>\n'+escHtml(lead):'',
    location?'📍 '+escHtml(location):'',
    url?'📌 <b>'+escHtml(labels.continueText)+'</b>\n'+escHtml(url):'',
    tags
  ].filter(Boolean);
  return lines.join('\n\n');
}
...
function rubikaText(a){
  ...
  if(lead){
    add('\n\n🔹 ');
    add(labels.summary,{type:'Bold'});
    add('\n'+lead);
  }
  if(location)add('\n\n📍 '+location);
  if(url){
    add('\n\n📌 ');
    add(labels.continueText,{type:'Bold'});
    add('\n');
...
async function updateExisting(a){
  const out={telegram:{status:'skipped'},rubika:{status:'skipped'}};

  const tg=a&&a.distribution&&a.distribution.telegram;
  if(tg&&tg.status==='sent'&&tg.messageId&&process.env.TELEGRAM_BOT_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    try{
      const base='https://api.telegram.org/bot'+process.env.TELEGRAM_BOT_TOKEN;
      const text=telegramText(a,'telegram');
      const method=(tg.mode==='photo'||tg.mode==='video')?'editMessageCaption':'editMessageText';
      const payload=method==='editMessageCaption'
        ?{chat_id:process.env.TELEGRAM_CHAT_ID,message_id:tg.messageId,caption:text.slice(0,1000),parse_mode:'HTML'}
        :{chat_id:process.env.TELEGRAM_CHAT_ID,message_id:tg.messageId,text,parse_mode:'HTML',disable_web_page_preview:false};
      await jsonPost(base+'/'+method,payload);
      out.telegram={status:'updated',messageId:tg.messageId,at:new Date().toISOString()};
    }catch(err){
      out.telegram={status:'failed',messageId:tg.messageId,error:String(err&&err.message||err)};
    }
  }

  const rb=a&&a.distribution&&a.distribution.rubika;
  if(rb&&rb.status==='sent'&&rb.messageId&&process.env.RUBIKA_BOT_TOKEN&&process.env.RUBIKA_CHAT_ID){
    try{
      await rubikaCall(process.env.RUBIKA_BOT_TOKEN,'editMessageText',{
        chat_id:process.env.RUBIKA_CHAT_ID,
        message_id:rb.messageId,
        text:rubikaCaption(a)
      });
      out.rubika={status:'updated',messageId:rb.messageId,at:new Date().toISOString()};
    }catch(err){
      out.rubika={status:'failed',messageId:rb.messageId,error:String(err&&err.message||err)};
    }
  }
  return out;
}
...
module.exports={status,dispatch,updateExisting,articleUrl,sharePackage};
