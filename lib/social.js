...
function telegramText(a,source='telegram'){
  ...
  const lines=[
    ...
    url?'📌 ادامه خبر را از لینک زیر در سایت نبض ساردو بخوانید:<br>🔗 '+escHtml(url):'',
    tags
  ].filter(Boolean);
  return lines.join('\n\n');
}
function plainText(a,source='site'){
  ...
  return [
    ...
    lead,
    location?'📍 '+location:'',
    url?'📌 ادامه خبر را از لینک زیر در سایت نبض ساردو بخوانید:\n🔗 '+url:'',
    tags
  ].filter(Boolean).join('\n\n').slice(0,3500);
}
function utf16Len(s=''){
...
function rubikaText(a){
  ...
  if(location)add('\n\n📍 '+location);
  if(url){
    add('\n\n📌 ادامه خبر را از لینک زیر در سایت نبض ساردو بخوانید:');
    add('\n🔗 ');
