...
async function liveConnectionTest(){
  const out={};
  const at=new Intl.DateTimeFormat('fa-IR',{dateStyle:'medium',timeStyle:'short',timeZone:'Asia/Tehran'}).format(new Date());
  const text='🧪 تست اتصال نبض ساردو\n\nاین پیام فقط برای بررسی ارسال خودکار از سایت نبض ساردو است.\n\n🕒 '+at;
  try{
    const token=process.env.TELEGRAM_BOT_TOKEN,chat=process.env.TELEGRAM_CHAT_ID;
    if(!token||!chat)out.telegram={status:'skipped',reason:'not-configured'};
    else{
      await jsonPost('https://api.telegram.org/bot'+token+'/sendMessage',{chat_id:chat,text,disable_web_page_preview:true});
      out.telegram={status:'sent',at:new Date().toISOString()};
    }
  }catch(err){out.telegram={status:'failed',error:String(err&&err.message||err)};}
  try{
    const token=process.env.RUBIKA_BOT_TOKEN,chat=process.env.RUBIKA_CHAT_ID;
    if(!token||!chat)out.rubika={status:'skipped',reason:'not-configured'};
    else{
      await rubikaCall(token,'sendMessage',{chat_id:chat,text});
      out.rubika={status:'sent',at:new Date().toISOString()};
    }
  }catch(err){out.rubika={status:'failed',error:String(err&&err.message||err)};}
  return out;
}
module.exports={status,dispatch,articleUrl,sharePackage,liveConnectionTest};
