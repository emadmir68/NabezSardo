...
async function rubika(a){
  ...
  await jsonPost(url,{chat_id:chat,text:postText(a,'rubika')});
  return {status:'sent',at:new Date().toISOString(),mode:'text'};
}
...
async function discoverRubikaChatIds(){
  const token=process.env.RUBIKA_BOT_TOKEN;
  if(!token||process.env.RUBIKA_CHAT_ID)return [];
  try{
    const res=await fetch('https://botapi.rubika.ir/v3/'+encodeURIComponent(token)+'/getUpdates',{
      method:'POST',
    ...
    for(const u of updates){
      if(!u||typeof u!=='object'||!u.chat_id)continue;
      const id=String(u.chat_id);
      const type=String(u.type||'');
      if(!found.some(x=>x.chat_id===id))found.push({chat_id:id,type});
    }
    return found;
  }catch(err){
    console.warn('rubika chat discovery failed',String(err&&err.message||err));
    return [];
  }
}
...
module.exports={status,dispatch,articleUrl,discoverRubikaChatIds};