...
function admin(db,backups=[],params=new URLSearchParams(),integrations={}){
 ...
 const siteTemplate=db.settings?.siteTemplate==='modern'?'modern':'classic';
 const msg=params.get('themeSaved')==='1'?'<div class="success">قالب سایت تغییر کرد و برای همه بازدیدکننده‌ها اعمال شد.</div>':params.get('adSaved')==='1'?'<div class="success">تبلیغ ذخیره شد و روی صفحه اصلی اعمال شد.</div>':params.get('adError')==='1'?'<div class="error">ذخیره تبلیغ انجام نشد؛ فرمت یا حجم تصویر را بررسی کنید.</div>':params.get('breakingSaved')==='1'?'<div class="success">متن نبض فوری ذخیره شد و روی سایت اعمال شد.</div>':params.get('backup')==='1'?'<div class="success">نسخه پشتیبان دستی ساخته شد.</div>':params.get('restored')==='1'?'<div class="success">نسخه پشتیبان با موفقیت بازیابی شد.</div>':params.get('restoreError')==='1'?'<div class="error">فایل پشتیبان معتبر نبود و بازیابی انجام نشد.</div>':'';
 ...
 <section class="panel template-admin-panel">
   <div class="panel-head"><div><h2>قالب سایت</h2><p>ظاهر عمومی سایت را انتخاب کن. خبرها، آدرس‌ها، سئو و پنل تحریریه با تغییر قالب دست‌نخورده می‌مانند.</p></div><span class="template-current-badge">${siteTemplate==='modern'?'قالب ۲ فعال':'قالب ۱ فعال'}</span></div>
   <form method="post" action="/admin/settings/template" class="template-picker">
     <label class="template-option ${siteTemplate==='classic'?'selected':''}">
       <input type="radio" name="siteTemplate" value="classic" ${siteTemplate==='classic'?'checked':''}>
       <span class="template-preview template-preview-classic" aria-hidden="true"><i></i><b></b><em></em><small></small></span>
       <span class="template-option-copy"><strong>قالب ۱ — لوکس تیره</strong><small>طراحی فعلی نبض ساردو؛ مشکی، طلایی و زرشکی با حس پریمیوم.</small></span>
     </label>
     <label class="template-option ${siteTemplate==='modern'?'selected':''}">
       <input type="radio" name="siteTemplate" value="modern" ${siteTemplate==='modern'?'checked':''}>
       <span class="template-preview template-preview-modern" aria-hidden="true"><i></i><b></b><em></em><small></small></span>
       <span class="template-option-copy"><strong>قالب ۲ — تحریریه روشن</strong><small>سبک خبرگزاری مدرن؛ روشن، خوانا، سریع و مناسب موبایل و خبرهای روز.</small></span>
     </label>
     <div class="template-picker-actions"><button class="btn gold" type="submit">ذخیره و اعمال قالب</button><a class="btn ghost" href="/" target="_blank" rel="noopener">پیش‌نمایش سایت</a></div>
   </form>
 </section>

 <section class="panel breaking-admin-panel"><div class="panel-head"><div><h2>نبض فوری</h2><p>متن نوار متحرک صفحه اصلی را از همین‌جا تغییر بده؛ بعد از ذخیره، بدون نیاز به تغییر کد روی سایت نمایش داده می‌شود.</p></div></div>
 <form method="post" action="/admin/settings/breaking" class="form breaking-admin-form">
   ...
   <button class="btn gold">ذخیره متن نبض فوری</button>
 </form></section>
