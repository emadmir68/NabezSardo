...
function admin(db,backups=[],params=new URLSearchParams(),integrations={},traffic={}){
 ...
 const msg=params.get('briefSaved')==='1'?'<div class="success">خبر کوتاه با موفقیت منتشر شد.</div>':params.get('briefError')==='1'?'<div class="error">عنوان خبر کوتاه نمی‌تواند خالی باشد.</div>':params.get('adSaved')==='1'?'<div class="success">تبلیغ ذخیره شد و روی صفحه اصلی اعمال شد.</div>':params.get('adError')==='1'?'<div class="error">ذخیره تبلیغ انجام نشد؛ فرمت یا حجم تصویر را بررسی کنید.</div>':params.get('breakingSaved')==='1'?'<div class="success">متن نبض فوری ذخیره شد و روی سایت اعمال شد.</div>':params.get('backup')==='1'?'<div class="success">نسخه پشتیبان دستی ساخته شد.</div>':params.get('restored')==='1'?'<div class="success">نسخه پشتیبان با موفقیت بازیابی شد.</div>':params.get('restoreError')==='1'?'<div class="error">فایل پشتیبان معتبر نبود و بازیابی انجام نشد.</div>':'';
 ...
 ${analyticsPanel(db,a,traffic)}
 <div class="stats editorial-stats"><div><b>${a.length}</b><span>کل خبرها</span></div><div><b>${a.filter(x=>x.status==='published').length}</b><span>منتشرشده</span></div><div><b>${a.filter(x=>x.status==='scheduled').length}</b><span>زمان‌بندی‌شده</span></div><div><b>${db.citizens.length}</b><span>خبر مردمی</span></div></div>

 <section class="panel quick-brief-admin">
   <div class="panel-head"><div><h2>ثبت سریع خبر کوتاه</h2><p>برای خبرهای فوری و یک/دو خطی؛ بدون ورود به فرم کامل.</p></div><a class="btn ghost" href="/briefs" target="_blank" rel="noopener">مشاهده صفحه خبر کوتاه</a></div>
   <form method="post" action="/admin/briefs/new" class="form quick-brief-form">
     <label>تیتر کوتاه<input name="title" maxlength="180" required placeholder="مثلاً: بارش باران در ارتفاعات ساردوئیه آغاز شد"></label>
     <label>توضیح یک یا دو خطی<textarea name="lead" maxlength="360" rows="3" placeholder="خلاصه خبر را کوتاه و مستقیم بنویسید"></textarea></label>
     <div class="twocol">
       <label>دسته‌بندی<select name="categoryId">${db.categories.map(c=>`<option value="${c.id}">${e(c.name)}</option>`).join('')}</select></label>
       <label>محل خبر<input name="location" maxlength="120" value="ساردوئیه"></label>
     </div>
     <div class="quick-brief-actions"><small>با زدن «انتشار فوری»، خبر مستقیم وارد صفحه «خبر کوتاه» می‌شود.</small><button class="btn primary">انتشار فوری</button></div>
   </form>
 </section>

 <section class="panel breaking-admin-panel"><div class="panel-head"><div><h2>LIVE DESK / نبض فوری</h2><p>وضعیت میز خبر زنده، متن فوری و زمان آخرین به‌روزرسانی را از همین بخش کنترل کن.</p></div><div class="live-desk-admin-state ${db.settings?.liveDeskEnabled!==false?'on':'off'}"><i></i><div><b>${db.settings?.liveDeskEnabled!==false?'LIVE DESK فعال':'LIVE DESK خاموش'}</b><small>${db.settings?.breakingUpdatedAt?'آخرین تغییر: '+fmt(db.settings.breakingUpdatedAt):'هنوز زمان تغییر ثبت نشده'}</small></div></div></div>
 ...
 <section class="panel ad-admin-panel">
...
function editor(db,a={},action='/admin/articles/new',title='خبر جدید'){
 ...
 <div class="twocol"><label>خبر ویژه<select name="featured"><option value="0">خیر</option><option value="1" ${a.featured?'selected':''}>بله</option></select></label><label>نمایش در «خبر کوتاه»<select name="shortNews"><option value="0">خیر</option><option value="1" ${a.shortNews?'selected':''}>بله — یک/دو خطی</option></select><small>برای خبرهای فوری و کوتاه؛ تیتر و لید در صفحه «خبر کوتاه» نمایش داده می‌شود.</small></label></div>
