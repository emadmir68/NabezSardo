...
function admin(db,backups=[],params=new URLSearchParams(),integrations={},traffic={}){
 ...
 const msg=params.get('saved')==='1'?'<div class="success">خبر ذخیره شد. ارسال شبکه‌ها در صف است و معمولاً طی یک دقیقه شروع می‌شود؛ وضعیت هر شبکه را در فهرست خبرها ببینید.</div>':params.get('followupSaved')==='1'?'<div class="success">پرونده پیگیری ذخیره شد و روی سایت اعمال شد.</div>':params.get('followupDeleted')==='1'?'<div class="success">پرونده پیگیری حذف شد.</div>':params.get('adSaved')==='1'?'<div class="success">تبلیغ ذخیره شد و روی صفحه اصلی اعمال شد.</div>':params.get('adError')==='1'?'<div class="error">ذخیره تبلیغ انجام نشد؛ فرمت یا حجم تصویر را بررسی کنید.</div>':params.get('breakingSaved')==='1'?'<div class="success">متن نبض فوری ذخیره شد و روی سایت اعمال شد.</div>':params.get('backup')==='1'?'<div class="success">نسخه پشتیبان دستی ساخته شد.</div>':params.get('restored')==='1'?'<div class="success">نسخه پشتیبان با موفقیت بازیابی شد.</div>':params.get('restoreError')==='1'?'<div class="error">فایل پشتیبان معتبر نبود و بازیابی انجام نشد.</div>':'';
...
function shareKit(db,a,params=new URLSearchParams()){
 ...
 const state=(key,label)=>{
   ...
   const text=s==='sent'?'ارسال شد':s==='partial'?'ارسال ناقص؛ جزئیات بررسی شود':s==='failed'?'خطا':s==='skipped'?'تنظیم نشده':s==='disabled'?'غیرفعال':s==='unknown'?'نامشخص؛ کانال را بررسی کنید':s==='sending'?'در حال ارسال':'در صف';
   return `<span class="share-kit-state" data-state="${e(s)}"><b>${e(label)}</b><small>${e(text)}</small></span>`;
 };
 const notice=params.get('resent')==='1'?'<div class="success">درخواست بررسی ارسال ثبت شد؛ شبکه‌های ارسال‌شده یا با نتیجه نامشخص دوباره ارسال نمی‌شوند.</div>':'';
...
function editor(db,a={},action='/admin/articles/new',title='خبر جدید'){
 ...
 <input type="hidden" name="submissionToken" value="${randomUUID()}">
