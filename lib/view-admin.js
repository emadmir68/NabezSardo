...
function admin(db,backups=[],params=new URLSearchParams(),integrations={},traffic={}){
 ...
 <div class="admin-dashboard-shell"><aside class="admin-sidebar"><div class="admin-sidebar-head"><span class="eyebrow">NEWSROOM</span><b>مدیریت تحریریه</b><small>نبض ساردو</small></div><nav class="admin-sidebar-nav"><a href="#overview"><span>نمای کلی</span></a><a href="#news"><span>مدیریت خبرها</span><i>${a.length}</i></a><a href="#live"><span>نبض فوری</span></a><a href="#ads"><span>تبلیغات</span></a><a href="#social"><span>شبکه‌های اجتماعی</span></a><a href="#citizens"><span>خبرهای مردمی</span><i>${db.citizens.length}</i></a><a href="#messages"><span>پیام‌ها</span><i>${db.contacts.length}</i></a><a href="#backup"><span>پشتیبان‌گیری</span></a></nav><a class="admin-sidebar-site" href="/">مشاهده سایت</a></aside><div class="admin-dashboard-content"><div class="admin-welcome"><div><span class="eyebrow">NEWSROOM ADMIN</span><h1>داشبورد تحریریه</h1><p>هر بخش جدا و مستقل است؛ برای مدیریت سریع‌تر از منوی کناری وارد همان قسمت شو.</p></div><a class="btn primary" href="/admin/articles/new">+ خبر جدید</a></div>
 <section id="overview" class="admin-section-block admin-overview"><div class="admin-section-title"><div><span class="admin-section-no">01</span><div><h2>نمای کلی</h2><p>خلاصه وضعیت تحریریه و آمار سایت</p></div></div></div><details class="admin-fold analytics-fold" open><summary><span><b>داشبورد هوشمند و عملکرد سایت</b><small>تحلیل زنده، پیشنهاد تحریریه، منابع ورودی و صفحات پربازدید</small></span><strong>باز و بسته کردن گزارش</strong></summary><div class="admin-fold-body">${analyticsPanel(db,a,traffic)}</div></details>
 <div class="stats editorial-stats admin-summary-stats"><div><b>${a.length}</b><span>کل خبرها</span></div><div><b>${a.filter(x=>x.status==='published').length}</b><span>منتشرشده</span></div><div><b>${a.filter(x=>x.status==='scheduled').length}</b><span>زمان‌بندی‌شده</span></div><div><b>${db.citizens.length}</b><span>خبر مردمی</span></div></div></section>
 ...
 <section id="news" class="panel admin-section-block admin-news-panel"><div class="admin-section-title compact"><div><span class="admin-section-no">02</span><div><h2>مدیریت خبرها</h2><p>ویرایش، حذف و بررسی وضعیت انتشار</p></div></div><a class="btn ghost" href="/admin/articles/new">خبر جدید</a></div><div class="admin-scroll-list">${a.map(x=>`<div class="adminrow"><div><b>${e(x.title)}</b><small>${statusLabel(x.status)} · ${x.status==='scheduled'&&x.scheduledAt?'انتشار: '+fmt(x.scheduledAt):fmt(x.createdAt)} · ${Number(x.views||0).toLocaleString('fa-IR')} بازدید</small><small class="distribution-mini">${['telegram','rubika','whatsapp'].map(k=>{const s=x.distribution?.[k]?.status;return k+': '+(s==='sent'?'✓':s==='failed'?'خطا':s==='skipped'?'—':'…')}).join(' · ')}</small></div><div><a href="/admin/articles/${x.id}/tv" class="admin-tv-link">🎬 نبض TV</a><a href="/admin/articles/${x.id}/edit">ویرایش</a><form method="post" action="/admin/articles/${x.id}/delete" data-confirm="این خبر حذف شود؟"><button>حذف</button></form></div></div>`).join('')||'<p>هنوز خبری ثبت نشده است.</p>'}</div></section>
...
function cleanPreviewText(v='',max=220){
  return String(v||'').replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim().slice(0,max);
}
function tvStudio(db,a,params=new URLSearchParams()){
  const duration=params.get('duration')==='30'?30:15;
  const category=db.categories.find(c=>c.id===a.categoryId)||{};
  const lead=cleanPreviewText(a.lead||a.body||a.title,260);
  const title=cleanPreviewText(a.title,180);
  const image=String(a.image||'');
  const status=statusLabel(a.status);
  const publicLink='/news/'+encodeURIComponent(a.slug||'');
  const imageMarkup=image
    ? '<div class="tv-media-bg" style="background-image:url(&quot;'+e(image)+'&quot;)"></div><img class="tv-media-img" src="'+e(image)+'" alt="">'
    : '<div class="tv-media-empty"><span>نبض ساردو</span><small>کاور خبر در زمان انتشار از قالب آماده موضوعی استفاده می‌کند</small></div>';

  let html='';
  html+='<div class="adminbar">'+logo()+'<a href="/admin">داشبورد</a><a href="/admin/articles/'+a.id+'/edit">ویرایش خبر</a></div>';
  html+='<main class="wrap admin tv-studio" data-tv-studio>';
  html+='<div class="tv-studio-head"><div><span class="eyebrow">NABEZ TV · PREVIEW STUDIO</span><h1>استودیوی نبض TV</h1><p>پیش‌نمایش آزمایشی و کاملاً جدا از سایت عمومی؛ فعلاً هیچ فایل MP4 ساخته نمی‌شود.</p></div>';
  html+='<div class="tv-studio-head-actions"><a class="btn ghost" href="/admin">بازگشت به داشبورد</a><a class="btn ghost" href="'+publicLink+'" target="_blank" rel="noopener">مشاهده خبر</a></div></div>';

  html+='<div class="tv-studio-grid"><section class="tv-preview-panel">';
  html+='<div class="tv-preview-top"><div><b>پیش‌نمایش 9:16</b><small>'+duration+' ثانیه · '+e(status)+'</small></div>';
  html+='<div class="tv-duration-switch" aria-label="مدت ویدیو"><a class="'+(duration===15?'active':'')+'" href="/admin/articles/'+a.id+'/tv?duration=15">15s</a><a class="'+(duration===30?'active':'')+'" href="/admin/articles/'+a.id+'/tv?duration=30">30s</a></div></div>';

  html+='<div class="tv-device"><div class="tv-frame" data-duration="'+duration+'" style="--tv-total:'+duration+'s">';
  html+='<div class="tv-progress"><i></i></div>';
  html+='<div class="tv-scene tv-scene-intro"><div class="tv-brand-pulse"><i></i><b>نبض ساردو</b></div><strong>NABEZ TV</strong><small>خبر در حرکت</small></div>';
  html+='<div class="tv-scene tv-scene-story"><div class="tv-media">'+imageMarkup+'</div><div class="tv-story-shade"></div><div class="tv-story-copy"><span>'+e(category.name||'خبر')+'</span><h2>'+e(title)+'</h2><small>نبض ساردو · رسانه محلی جنوب کرمان</small></div></div>';
  html+='<div class="tv-scene tv-scene-summary"><span>خلاصه خبر</span><h3>'+e(title)+'</h3><p>'+e(lead)+'</p></div>';
  html+='<div class="tv-scene tv-scene-outro"><div class="tv-outro-mark">N</div><b>ادامه خبر در نبض ساردو</b><strong>nabzesardo.ir</strong><small>ساردوئیه · جیرفت · جنوب کرمان</small></div>';
  html+='<div class="tv-safe-top"><span>نبض TV</span><b>'+duration+'s</b></div>';
  html+='</div></div><p class="tv-preview-note">پیش‌نمایش به‌صورت خودکار تکرار می‌شود. برای مقایسه، 15 یا 30 ثانیه را انتخاب کن.</p></section>';

  html+='<aside class="tv-control-panel">';
  html+='<div class="tv-control-card ready"><span>01</span><div><b>منبع تصویر</b><p>'+(image?'عکس واقعی همین خبر استفاده می‌شود.':'این خبر عکس ندارد؛ در خروجی از کاور آماده موضوعی استفاده می‌شود.')+'</p></div></div>';
  html+='<div class="tv-control-card ready"><span>02</span><div><b>متن ویدیو</b><p>تیتر و خلاصه فقط از محتوای همین خبر گرفته می‌شوند؛ متن یا تصویر AI ساخته نمی‌شود.</p></div></div>';
  html+='<div class="tv-control-card ready"><span>03</span><div><b>فرمت نهایی</b><p>عمودی 1080×1920، مناسب استوری، ریلز، روبیکا و وضعیت واتساپ.</p></div></div>';
  html+='<div class="tv-control-card pending"><span>04</span><div><b>رندر MP4</b><p>عمداً هنوز فعال نشده. بعد از تأیید ظاهر این پیش‌نمایش، موتور رندر واقعی را متصل می‌کنیم.</p></div></div>';
  html+='<div class="tv-rollback-box"><b>نسخه آزمایشی و قابل بازگشت</b><p>تمام تغییرات نبض TV جدا از صفحه اصلی و صفحات خبر انجام شده‌اند. اگر ظاهر را نپسندی، همین قابلیت را بدون دست‌زدن به بقیه سایت برمی‌گردانیم.</p></div>';
  html+='</aside></div></main>';
  return shell('نبض TV | '+title,html);
}

module.exports={login,admin,editor,tvStudio};